...
common_siege_ai_trigger_init_after_2_secs = ( ## we put it back so archers in the begining will stay where they spawn, they will later move if they do not use their ammo
  ...
  [
    (try_for_agents, ":agent_no"),
	  (agent_get_division, ":class", ":agent_no"),
  ...
  [#refill ammo every 1 minute 30 sec.
    ...
    (try_for_agents,":cur_agent"),
	  ...
	  (try_begin),
		...
		(agent_get_ammo, ":old_ammo", ":cur_agent", 0),
		(ge, ":old_ammo", 0),
		
		(agent_refill_ammo, ":cur_agent"),
		(agent_get_ammo, ":new_ammo", ":cur_agent", 1),
		(store_sub, ":diff", ":new_ammo", ":old_ammo"),
		# Apparently there is a bug with recharging ammo other than arrows that gives one ammo to the agent when he is wielding no ranged weapon
		# If the agent has used only one ammo he is probably in a bad spot anyways
		(le, ":diff", 1),
  ...
  [
	...
	(try_for_agents, ":agent_no"),
	  ...
	  (try_begin), ## change infantries with ranged weapon to archers to search for good shoting position
		...
		(gt, ":ammo", 0),
		(try_begin),
		  (eq, ":division", grc_infantry), # infantry
		  (val_add, ":difference", -1),
		(else_try),
		  (eq, ":division", grc_cavalry), # cavalry
		  (val_add, ":difference", 1),
		(try_end),
		(val_add, ":num_archers", 1),
