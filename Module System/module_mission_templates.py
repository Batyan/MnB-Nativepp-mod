...
field_ai_triggers = [
	...
	(try_for_agents, ":agent"), # Run through all active NPCs on the battle field.
     ...
     # Still mounted
                ...
                (else_try),
					...
					(neg|is_between, ":wielded", ranged_weapons_begin, ranged_weapons_end), # do not switch if using a ranged weapon
  ...
  (ti_on_agent_dismount, 0, 0, [(party_get_slot, reg3, "p_main_party", slot_party_pref_div_dehorse),(is_between, reg3, 0, 9)], #De-Horse Trigger #Valid division 0-8
   [
	...
	(try_end),
	(agent_force_rethink, ":rider"), # to force ranged weapon users to switch weapon if ennemies nearby
   ]),
...
common_siege_ai_trigger_init_2 = (
  ...
  [
	...
	(assign, "$g_siege_division_double_entry", 0),
	(entry_point_get_position, pos9, 9),
	(try_for_agents, ":cur_agent"),
	  (agent_is_alive, ":cur_agent"),
	  (agent_is_human, ":cur_agent"),
      (agent_get_team, ":agent_team", ":cur_agent"),
	  ...
	  (agent_set_division, ":cur_agent", "$g_siege_division_double_entry"),
	  (val_add, "$g_siege_division_double_entry", 2),
	  (val_mod, "$g_siege_division_double_entry", 4),
	(try_end),
	(team_set_order_position, "$defender_team", 2, pos9),
	(team_set_order_position, "$defender_team_2", 2, pos9),
    (set_show_messages, 1),
    ], [])

common_siege_ai_trigger_init_after_2_secs = ( ## we put it back so archers in the begining will stay where they spawn, they will later move if they do not use their ammo
  0, 2, ti_once, [],
  [
    (try_for_agents, ":agent_no"),
	  (agent_get_class, ":class", ":agent_no"),
	  (eq, ":class", grc_archers), # only for archers, the others will still be moving in the begining because they don't spawn in archer positions
...
common_siege_defender_reinforcement_check = (
  3, 0, 5, [],
  [# (lt, "$defender_reinforcement_stage", 9), # defenders will keep spawning if they have a lot of troops
   (store_mission_timer_a,":mission_time"),
   (ge,":mission_time",10),
   (store_normalized_team_count,":num_defenders",0),
   (try_begin),
     (eq, "$test", 1),
	 (assign, reg15, ":num_defenders"),
	 (display_message, "@Normalized defenders : {reg15}"),
   (try_end),
   (lt,":num_defenders",12),
   (add_reinforcements_to_entry, 4, 5),
...
common_siege_defender_reinforcement_archer_reposition = (
  2, 0, 0,
  [
    # (gt, "$defender_reinforcement_stage", 0), # we want them to keep getting to shoting position even if this is not the first wave !
    ],
  [
...
common_siege_attacker_reinforcement_check = (
  ...
  [
    (add_reinforcements_to_entry, 1, 5),
...
common_siege_refill_ammo = (
  90, 0, 0, [],
  [#refill ammo every 1 minute 30 sec.
    (get_player_agent_no, ":player_agent"),
    (try_for_agents,":cur_agent"),
      ...
      (agent_is_human, ":cur_agent"),
	  (try_begin),
	    # (eq, "$test", 1), # it is working properly and is helpful for the starting archers, they will no longer spawn and go in another spawn position
		(agent_get_team, ":agent_team", ":cur_agent"),
        (this_or_next|eq, ":agent_team", "$defender_team"),
        (eq, ":agent_team", "$defender_team_2"),
		# only change positions of defending bots

		(agent_slot_eq, ":cur_agent", slot_agent_is_in_scripted_mode, 0), # is not moving to another place
		...
		(agent_get_wielded_item, ":wielded", ":cur_agent", 0),
		(try_begin),
		  (is_between, ":wielded", "itm_hunting_bow", "itm_hunting_crossbow"),
		  (agent_get_ammo, ":ammo", ":cur_agent", 1),
		  (ge, ":ammo", 35),
		  (agent_set_slot, ":cur_agent", slot_agent_is_not_reinforcement, 0),
		  (agent_set_slot, ":cur_agent", slot_agent_target_entry_point, 0), # to reassign it to another random value later
		  # change position if no ammo has been used -- defensive position overused or agent has bad placement
		(else_try),
		  (is_between, ":wielded", "itm_hunting_crossbow", "itm_flintlock_pistol"),
		  (agent_get_ammo, ":ammo", ":cur_agent", 1),
		  # (assign, reg15, ":ammo"),
		  (ge, ":ammo", 25),
		  (agent_set_slot, ":cur_agent", slot_agent_is_not_reinforcement, 0),
		  (agent_set_slot, ":cur_agent", slot_agent_target_entry_point, 0), # to reassign it to another random value
		  # change position if no ammo has been used -- defensive position overused or agent has bad placement
		(else_try),
		  (this_or_next|is_between, ":wielded", "itm_darts", "itm_javelin"),
		  (is_between, ":wielded", "itm_throwing_knives", "itm_light_throwing_axes"),
		  (agent_get_ammo, ":ammo", ":cur_agent", 1),
		  # (assign, reg15, ":ammo"),
		  (ge, ":ammo", 7),
		  (agent_set_slot, ":cur_agent", slot_agent_is_not_reinforcement, 0),
		  ...
		  # change position if no ammo has been used -- defensive position overused or agent has bad placement
		(else_try),
		  (is_between, ":wielded", "itm_javelin", "itm_stones"),
		  (agent_get_ammo, ":ammo", ":cur_agent", 1),
		  # (assign, reg15, ":ammo"),
		  (ge, ":ammo", 5),
		  (agent_set_slot, ":cur_agent", slot_agent_is_not_reinforcement, 0),
		  ...
		  # change position if no ammo has been used -- defensive position overused or agent has bad placement
		(else_try),
		  (is_between, ":wielded", "itm_light_throwing_axes", "itm_hunting_bow"),
		  (agent_get_ammo, ":ammo", ":cur_agent", 1),
		  # (assign, reg15, ":ammo"),
		  (ge, ":ammo", 4),
		  (agent_set_slot, ":cur_agent", slot_agent_is_not_reinforcement, 0),
...
common_battle_combat_skill = ( ## disabled for now
  ti_on_agent_hit, 0, 0,
  [ (eq, 0, 1),
    (is_between, reg0, weapons_begin, weapons_end), # attack must be with a melee weapon
	(store_trigger_param_1, ":victim"),
	# (store_trigger_param_2, ":attacker"),
	# (gt, reg10, 0), # attack has to deal damages
	# (agent_get_wielded_item, ":shield", ":victim", 1),
	# (is_between, ":shield", shields_begin, shields_end), # defender must have a shield
	(agent_get_horse, ":horse", ":victim"), # no horseman
	(le, ":horse", 0),
  ],
  [
    (store_trigger_param_1, ":victim"),
	...
	(store_mission_timer_b_msec, ":timer"),
	(try_begin),
	  (gt, ":damage", 1), # attack has to deal damages
	  (agent_get_troop_id, ":troop_id", ":victim"),
      (this_or_next|troop_is_hero, ":troop_id"),
	  (is_between, ":troop_id", noble_troops_begin, noble_troops_end),
	  (agent_set_defend_action, ":victim", 0, 800), # defend for 0.8 seconds (might be to long for fast weapons - too short for slow ones)
	  (agent_set_slot, ":victim", slot_agent_combat_stance, 1),
	  (agent_set_slot, ":victim", slot_agent_min_check_time, 800),
	  (agent_set_slot, ":victim", slot_agent_timer_depleted, 0),
	  (agent_set_slot, ":victim", slot_agent_timer_start, ":timer"),

	  (agent_set_slot, ":attacker", slot_agent_combat_stance, 2),
	  (agent_set_slot, ":attacker", slot_agent_min_check_time, 400),
	  (agent_set_slot, ":attacker", slot_agent_timer_start, ":timer"),
	  (agent_set_slot, ":attacker", slot_agent_timer_depleted, 0),

	(else_try),
	  (agent_get_troop_id, ":troop_id", ":attacker"),
      (this_or_next|troop_is_hero, ":troop_id"),
	  (is_between, ":troop_id", noble_troops_begin, noble_troops_end),
	  (agent_set_slot, ":attacker", slot_agent_combat_stance, 1),
	  (agent_set_defend_action, ":attacker", 0, 800), # defend for 0.8 seconds (might be to long for fast weapons - too short for slow ones)
	  (agent_set_slot, ":attacker", slot_agent_min_check_time, 800),
	  (agent_set_slot, ":attacker", slot_agent_timer_start, ":timer"),
	  (agent_set_slot, ":attacker", slot_agent_timer_depleted, 0),
	(try_end),
   ])

common_battle_combat_skill_2 = ( ## disabled for now
  0, 0, 0,
  [(eq, 0, 1),
  ],
  [
   (try_for_agents, ":agent_no"),
	 ...
	 (is_between, ":weapon", weapons_begin, weapons_end),
	 (agent_get_horse, ":horse", ":agent_no"), # no horseman
	 (le, ":horse", 0),
	 
	 (store_mission_timer_b_msec, ":timer"),
	 
	 (agent_get_slot, ":state", ":agent_no", slot_agent_combat_stance),
	 (agent_get_combat_state, ":combat_stance", ":agent_no"),
	 
	 (agent_get_slot, ":defend_time", ":agent_no", slot_agent_min_check_time),
	 (agent_get_slot, ":timer_start", ":agent_no", slot_agent_timer_start),
	 (store_add, ":total_timer", ":defend_time", ":timer_start"),
	 (val_mod, ":total_timer", 1000),
	 (store_mod, ":timer_mod_1000", ":timer", 1000),
	 (try_begin),
	   (eq, ":total_timer", ":timer_mod_1000"),
	   (agent_set_slot, ":agent_no", slot_agent_timer_depleted, 1),
	 (try_end),
	 (agent_get_slot, ":continue", ":agent_no", slot_agent_timer_depleted),
	 
	 (agent_get_attack_action, ":attack_state", ":agent_no"),
	 (try_begin),
	   (eq, ":state", 2),
	   (eq, ":combat_stance", 0),
	   (eq, ":continue", 1), # only check if end attack after 0.4 sec to allow them to feint
	   (agent_set_slot, ":agent_no", slot_agent_combat_stance, 1),
	   (agent_set_slot, ":agent_no", slot_agent_timer_depleted, 1),
     (else_try),
	   (eq, ":state", 1),
	   (eq, ":continue", 1),
	   (agent_set_slot, ":agent_no", slot_agent_combat_stance, 0),
	   (agent_set_slot, ":agent_no", slot_agent_min_check_time, 800),
	   (agent_set_slot, ":agent_no", slot_agent_timer_start, ":timer"),
	   (agent_set_slot, ":agent_no", slot_agent_timer_depleted, 0),
	   (agent_set_defend_action, ":agent_no", 0, 800),
	 (else_try),
	   (eq, ":state", 0),
	   (eq, ":attack_state", 2),
	   (agent_set_slot, ":agent_no", slot_agent_combat_stance, 2),
     (try_end),
   (try_end),
  ])
  ...
  [
	...
	(try_for_agents, ":agent_no"),
	  ...
	  (try_begin), ## change infantries with ranged weapon to archers to search for good shoting position
		...
		(else_try),
		  (eq, ":division", 2), # cavalry
		  (val_add, ":difference", 1),
		(try_end),
		(val_add, ":num_archers", 1),
		(agent_set_division, ":agent_no", 1), ## assign as archer
	  ...
	  (else_try),
		...
		(agent_get_ammo, ":ammo", ":agent_no", 0),	## will make sarranid troops move a lot when defending
		(try_begin),
		  (lt, ":ammo", 1),							## as they have throwing weapons => low ammo
		  (agent_set_division, ":agent_no", 0),		## => switch between foot soldier and archer
...
##          (agent_get_team, reg3, ":dead_agent_no"),          
      ...
      (0, 0, ti_once, [], [(assign,"$g_battle_won",0),
                           ...
                           (call_script, "script_combat_music_set_situation_with_culture"),
                           (assign, "$g_defender_reinforcement_limit", 3),
                           ]),
    ...
    ] + common_pbod_triggers + prebattle_orders_triggers + prebattle_deployment_triggers + caba_order_triggers + field_ai_triggers + custom_camera_triggers + AI_triggers + formations_triggers,
  ),
...
##  ),
  ...
  (
    ...
    [
      ...
      common_battle_victory_display,
      custom_battle_check_defeat_condition,
    ] + AI_triggers + formations_triggers
  ),
...
##  ),
    ...
    [
      ...
      (1, 0, ti_once, [],
	   ...
	   [          
		  ...
		  (store_div, ":number_of_veteran_bandits_will_be_spawned_at_each_period", ":number_of_bandits_will_be_spawned_at_each_period", 5),
		  (val_max, ":number_of_veteran_bandits_will_be_spawned_at_each_period", 0),
          ...
          (try_begin),
			...
			(assign, ":veteran_bandit_troop", "trp_skull_crusher"),
          (else_try),
            (eq, ":template", "pt_forest_bandit_lair"),
            (eq, ":random_value", 0),
            (assign, ":bandit_troop", "trp_forest_bandit"),
			(store_random_in_range, ":vet", 0, 2),
		    (try_begin),
		      (eq, ":vet", 0),
		      (assign, ":veteran_bandit_troop", "trp_forest_warrior"),
	        (else_try),
		      (assign, ":veteran_bandit_troop", "trp_forest_hunter"),
		    (try_end),
          (else_try),
			...
			(try_begin),
		      (eq, ":vet", 0),
		      (assign, ":veteran_bandit_troop", "trp_desert_rider"),
	        (else_try),
		      (assign, ":veteran_bandit_troop", "trp_desert_skirmisher"),
		    (try_end),
          (else_try),
		    ...
		    (try_begin),
		      (eq, ":vet", 0),
		      (assign, ":veteran_bandit_troop", "trp_mountain_raider"),
	        (else_try),
		      (assign, ":veteran_bandit_troop", "trp_mountain_cavalry"),
		    (try_end),
          (else_try),
            (eq, ":template", "pt_taiga_bandit_lair"),
            (eq, ":random_value", 0),
            (assign, ":bandit_troop", "trp_taiga_bandit"),
			(assign, ":veteran_bandit_troop", "trp_tundra_bandit"),
          (else_try),
            (eq, ":template", "pt_steppe_bandit_lair"),
            (eq, ":random_value", 0),
            (assign, ":bandit_troop", "trp_steppe_bandit"),
			(assign, ":veteran_bandit_troop", "trp_steppe_rider"),
          (else_try),
            (this_or_next|eq, ":template", "pt_looter_lair"),
            (neq, ":random_value", 0),
            (assign, ":bandit_troop", "trp_looter"),
			(assign, ":veteran_bandit_troop", "trp_bandit"),
          (try_end),
		  ...
		  (store_random_in_range, ":vet", 0, ":number_of_veteran_bandits_will_be_spawned_at_each_period"),
		  (try_begin),
		    (ge, ":number_of_veteran_bandits_will_be_spawned_at_each_period", 1),
		    (store_random_in_range, ":random_entry_point", 2, 11),
            (add_visitors_to_current_scene, ":random_entry_point", ":veteran_bandit_troop", 1),
			(val_add, "$bandits_spawned_extra", 1),
		  (try_end),
       ]),	   	   
      ...
      (ti_on_agent_killed_or_wounded, 0, 0, [],
       [
        ...
        (try_begin),
          ...
          (val_div, ":number_of_bandits_will_be_spawned_at_each_period", 3),
		  (store_div, ":number_of_veteran_bandits_will_be_spawned_at_each_period", ":number_of_bandits_will_be_spawned_at_each_period", 5),
		  (val_max, ":number_of_veteran_bandits_will_be_spawned_at_each_period", 1),
          (try_begin),
            (ge, "$defender_reinforcement_stage", 2),
  ...
  (
    ...
    [
      ...
      [      
        ...
        (get_distance_between_positions, ":dist", pos0, pos1),
        (ge, ":dist", 150), # changed this so the bandit can switch weapon
  ...
  (
	...
	[
		...
		(11,mtef_visitor_source|mtef_team_0,0,aif_start_alarmed,20,[]),
		(12,mtef_visitor_source|mtef_team_0,0,aif_start_alarmed,20,[]),
	...
	[
		(ti_before_mission_start, 0, 0, [], 
			[
				(assign, "$wave_no", 0),
				# (assign, "$reinforcement_left", 1),
				(assign, "$num_reinforcement", 1),
				(team_set_relation, 0, 1, -1),
				(store_random_in_range, ":ennemy_faction", "fac_kingdom_1", "fac_kingdom_10"),
				
				# (try_begin),
					# (eq, ":ennemy_faction", ":faction_ennemy"),
					# (assign, ":ennemy_faction", "fac_kingdom_7"),
				# (try_end),
				(try_begin),
					(eq, ":ennemy_faction", "fac_kingdom_1"),
					(assign, "$tier_1_inf", "trp_swadian_militia"),
					(assign, "$tier_2_inf", "trp_swadian_footman"),
					(assign, "$tier_3_inf", "trp_swadian_infantry"),
					(assign, "$tier_1_cav", "trp_swadian_cavalryman"),
					(assign, "$tier_2_cav", "trp_swadian_man_at_arms"),
					(assign, "$tier_1_arc", "trp_swadian_skirmisher"),
				(else_try),
					(eq, ":ennemy_faction", "fac_kingdom_2"),
					(assign, "$tier_1_inf", "trp_vaegir_footman"),
					(assign, "$tier_2_inf", "trp_vaegir_veteran"),
					(assign, "$tier_3_inf", "trp_vaegir_infantry"),
					(assign, "$tier_1_cav", "trp_vaegir_light_cavalry"),
					(assign, "$tier_2_cav", "trp_vaegir_horseman"),
					(assign, "$tier_1_arc", "trp_vaegir_skirmisher"),
				(else_try),
					(eq, ":ennemy_faction", "fac_kingdom_3"),
					(assign, "$tier_1_inf", "trp_khergit_skirmisher"),
					(assign, "$tier_2_inf", "trp_khergit_steppe_fighter"),
					(assign, "$tier_3_inf", "trp_khergit_soldier"),
					(assign, "$tier_1_cav", "trp_khergit_horseman"),
					(assign, "$tier_2_cav", "trp_khergit_dune_rider"),
					(assign, "$tier_1_arc", "trp_khergit_horse_archer"),
				(else_try),
					(eq, ":ennemy_faction", "fac_kingdom_4"),
					(assign, "$tier_1_inf", "trp_nord_footman"),
					(assign, "$tier_2_inf", "trp_nord_trained_footman"),
					(assign, "$tier_3_inf", "trp_nord_warrior"),
					(assign, "$tier_1_cav", "trp_nord_mounted_raider"),
					(assign, "$tier_2_cav", "trp_nord_veteran"),
					(assign, "$tier_1_arc", "trp_nord_huntsman"),
				(else_try),
					(eq, ":ennemy_faction", "fac_kingdom_6"),
					(assign, "$tier_1_inf", "trp_sarranid_footman"),
					(assign, "$tier_2_inf", "trp_sarranid_veteran_footman"),
					(assign, "$tier_3_inf", "trp_sarranid_infantry"),
					(assign, "$tier_1_cav", "trp_sarranid_cavalry"),
					(assign, "$tier_2_cav", "trp_sarranid_horseman"),
					(assign, "$tier_1_arc", "trp_sarranid_skirmisher"),
				(else_try),
					(eq, ":ennemy_faction", "fac_kingdom_5"),
					(assign, "$tier_1_inf", "trp_rhodok_swordman"),
					(assign, "$tier_2_inf", "trp_rhodok_trained_swordman"),
					(assign, "$tier_3_inf", "trp_rhodok_veteran_swordman"),
					(assign, "$tier_1_cav", "trp_rhodok_mounted_trained_skirmisher"),
					(assign, "$tier_2_cav", "trp_rhodok_veteran_spearman"),
					(assign, "$tier_1_arc", "trp_rhodok_crossbowman"),
				(else_try),
					#(eq, ":ennemy_faction", "fac_kingdom_7"),
					(assign, "$tier_1_inf", "trp_watchman"),
					(assign, "$tier_2_inf", "trp_town_watch"),
					(assign, "$tier_3_inf", "trp_mercenary_swordsman"),
					(assign, "$tier_1_cav", "trp_caravan_guard"),
					(assign, "$tier_2_cav", "trp_mercenary_horseman"),
					(assign, "$tier_1_arc", "trp_mercenary_fieldman"),
				(try_end),
				
				(store_random_in_range, ":faction_ennemy", "fac_kingdom_1", "fac_kingdom_10"),
				(try_begin),
					(eq, ":faction_ennemy", "fac_kingdom_1"),
					(assign, "$t_1_inf", "trp_swadian_militia"),
					(assign, "$t_2_inf", "trp_swadian_footman"),
					(assign, "$t_3_inf", "trp_swadian_infantry"),
					(assign, "$t_1_cav", "trp_swadian_cavalryman"),
					(assign, "$t_2_cav", "trp_swadian_man_at_arms"),
					(assign, "$t_1_arc", "trp_swadian_crossbowman"),
				(else_try),
					(eq, ":faction_ennemy", "fac_kingdom_2"),
					(assign, "$t_1_inf", "trp_vaegir_footman"),
					(assign, "$t_2_inf", "trp_vaegir_veteran"),
					(assign, "$t_3_inf", "trp_vaegir_infantry"),
					(assign, "$t_1_cav", "trp_vaegir_light_cavalry"),
					(assign, "$t_2_cav", "trp_vaegir_horseman"),
					(assign, "$t_1_arc", "trp_vaegir_archer"),
				(else_try),
					(eq, ":faction_ennemy", "fac_kingdom_3"),
					(assign, "$t_1_inf", "trp_khergit_steppe_fighter"),
					(assign, "$t_2_inf", "trp_khergit_soldier"),
					(assign, "$t_3_inf", "trp_khergit_dune_fighter"),
					(assign, "$t_1_cav", "trp_khergit_horseman"),
					(assign, "$t_2_cav", "trp_khergit_dune_rider"),
					(assign, "$t_1_arc", "trp_khergit_archer"),
				(else_try),
					(eq, ":faction_ennemy", "fac_kingdom_4"),
					(assign, "$t_1_inf", "trp_nord_footman"),
					(assign, "$t_2_inf", "trp_nord_trained_footman"),
					(assign, "$t_3_inf", "trp_nord_warrior"),
					(assign, "$t_1_cav", "trp_nord_mounted_raider"),
					(assign, "$t_2_cav", "trp_nord_veteran"),
					(assign, "$t_1_arc", "trp_nord_archer"),
				(else_try),
					(eq, ":faction_ennemy", "fac_kingdom_6"),
					(assign, "$t_1_inf", "trp_sarranid_footman"),
					(assign, "$t_2_inf", "trp_sarranid_veteran_footman"),
					(assign, "$t_3_inf", "trp_sarranid_infantry"),
					(assign, "$t_1_cav", "trp_sarranid_cavalry"),
					(assign, "$t_2_cav", "trp_sarranid_horseman"),
					(assign, "$t_1_arc", "trp_sarranid_archer"),
				(else_try),
					(eq, ":faction_ennemy", "fac_kingdom_5"),
					(assign, "$t_1_inf", "trp_rhodok_spearman"),
					(assign, "$t_2_inf", "trp_rhodok_trained_swordman"),
					(assign, "$t_3_inf", "trp_rhodok_veteran_swordman"),
					(assign, "$t_1_cav", "trp_rhodok_mounted_trained_skirmisher"),
					(assign, "$t_2_cav", "trp_rhodok_veteran_spearman"),
					(assign, "$t_1_arc", "trp_rhodok_veteran_crossbowman"),
				(else_try),
					(assign, "$t_1_inf", "trp_watchman"),
					(assign, "$t_2_inf", "trp_town_watch"),
					(assign, "$t_3_inf", "trp_mercenary_swordsman"),
					(assign, "$t_1_cav", "trp_caravan_guard"),
					(assign, "$t_2_cav", "trp_mercenary_horseman"),
					(assign, "$t_1_arc", "trp_mercenary_fieldman"),
				(try_end),
				(store_current_scene, ":cur_scene"),
				(modify_visitors_at_site, ":cur_scene"),
				(try_for_range, ":cur_entry_point", 1, 4),
					(add_visitors_to_current_scene, ":cur_entry_point", "$t_1_arc", 1),
				(try_end),
				(add_visitors_to_current_scene, 4, "$t_2_inf", 1),
				(add_visitors_to_current_scene, 5, "$t_2_inf", 1),
				(add_visitors_to_current_scene, 6, "$t_1_inf", 1),
				(add_visitors_to_current_scene, 7, "$t_3_inf", 1),
				(add_visitors_to_current_scene, 8, "$t_3_inf", 1),
				(add_visitors_to_current_scene, 9, "$t_3_inf", 1),
				(add_visitors_to_current_scene, 10, "$t_3_inf", 1),
				
				#(add_visitors_to_current_scene, 11, "$tier_3_inf", 1),
				#(add_visitors_to_current_scene, 12, "$tier_2_inf", 1),
		...
		(1, 0, 0, [(gt, "$reinforcement_left", 0),],
			[
				(store_current_scene, ":cur_scene"),
				(modify_visitors_at_site, ":cur_scene"),
				(try_for_range, ":unused", 0, 5),
					(store_random_in_range, ":random_entry_point", 11, 13),
					(store_random_in_range, ":rand", 0, 35),
					(try_begin),
						(lt, ":rand", 5),
						(assign, ":ennemy_troop", "$tier_1_inf"),
					(else_try),
						(lt, ":rand", 19),
						(assign, ":ennemy_troop", "$tier_2_inf"),
					(else_try),
						(lt, ":rand", 22),
						(assign, ":ennemy_troop", "$tier_3_inf"),
					(else_try),
						(lt, ":rand", 32),
						(assign, ":ennemy_troop", "$tier_1_cav"),
					(else_try),
						(lt, ":rand", 34),
						(assign, ":ennemy_troop", "$tier_2_cav"),
					(else_try),
						(lt, ":rand", 35),
						(assign, ":ennemy_troop", "$tier_1_arc"),
					(try_end),
					(add_visitors_to_current_scene, ":random_entry_point", ":ennemy_troop", 1),
				(try_end),
				(val_sub, "$reinforcement_left", 1),
		...
		(30, 0, 0,
			...
			[
				(display_message, "@Reinforcements!"),
				(store_current_scene, ":cur_scene"),
				(modify_visitors_at_site, ":cur_scene"),
				
				(try_for_range, ":unused", 0, "$num_reinforcement"),
					(store_random_in_range, ":rand", 0, 35),
					(try_begin),
						(lt, ":rand", 5),
						(assign, ":ennemy_troop", "$t_1_inf"),
					(else_try),
						(lt, ":rand", 19),
						(assign, ":ennemy_troop", "$t_2_inf"),
					(else_try),
						(lt, ":rand", 22),
						(assign, ":ennemy_troop", "$t_3_inf"),
					(else_try),
						(lt, ":rand", 28),
						(assign, ":ennemy_troop", "$t_1_cav"),
					(else_try),
						(lt, ":rand", 30),
						(assign, ":ennemy_troop", "$t_2_cav"),
					(else_try),
						(lt, ":rand", 35),
						(assign, ":ennemy_troop", "$t_1_arc"),
					(try_end),
					(add_visitors_to_current_scene, 10, ":ennemy_troop", 1),
				(try_end),
			]),
		...
		(10, 0, 0, 
			...
			[
				(val_add, "$wave_no", 1),
				(store_random_in_range, ":chance_increase", 0, 50),
				(try_begin),
					(eq, ":chance_increase", 0),
					(val_add, "$num_reinforcement", 2),
				(else_try),
					(le, ":chance_increase", 15),
					(val_add, "$num_reinforcement", 1),
				(try_end),
				(assign, "$reinforcement_left", "$num_reinforcement"),
				(store_mul, reg18, "$reinforcement_left", 5),
				(assign, reg17, "$wave_no"),
				(display_message, "@New wave, {reg18} ennemies (wave {reg17})."),
			]),
	  ...
	  # (ti_on_agent_spawn, 0, 0,
		...
		(2, 5, ti_once, 
			...
			[
				(assign, reg17, "$wave_no"),
				(display_message, "@You have been defeated at wave {reg17}."),
				(finish_mission),
			]),
	...
	[
		(ti_before_mission_start, 0, 0, [], 
			[
				...
				(assign, "$g_team_1_faction", "fac_kingdom_1"),
				(assign, "$g_team_2_faction", "fac_kingdom_2"),
				
				(faction_set_slot, "fac_kingdom_1", slot_faction_troop_1, "trp_swadian_militia"),
				(faction_set_slot, "fac_kingdom_1", slot_faction_troop_2, "trp_swadian_militia"),
				(faction_set_slot, "fac_kingdom_1", slot_faction_troop_3, "trp_swadian_footman"),
				(faction_set_slot, "fac_kingdom_1", slot_faction_troop_4, "trp_swadian_infantry"),
				(faction_set_slot, "fac_kingdom_1", slot_faction_troop_5, "trp_swadian_skirmisher"),
				(faction_set_slot, "fac_kingdom_1", slot_faction_troop_6, "trp_swadian_crossbowman"),
				(faction_set_slot, "fac_kingdom_1", slot_faction_troop_7, "trp_swadian_cavalryman"),
				(faction_set_slot, "fac_kingdom_1", slot_faction_troop_8, "trp_swadian_man_at_arms"),
				(faction_set_slot, "fac_kingdom_1", slot_faction_troop_9, "trp_swadian_noble3"),
				(faction_set_slot, "fac_kingdom_1", slot_faction_troop_10, "trp_swadian_noble4"),
				
				(faction_set_slot, "fac_kingdom_2", slot_faction_troop_1, "trp_vaegir_footman"),
				(faction_set_slot, "fac_kingdom_2", slot_faction_troop_2, "trp_vaegir_scout"),
				(faction_set_slot, "fac_kingdom_2", slot_faction_troop_3, "trp_vaegir_veteran"),
				(faction_set_slot, "fac_kingdom_2", slot_faction_troop_4, "trp_vaegir_infantry"),
				(faction_set_slot, "fac_kingdom_2", slot_faction_troop_5, "trp_vaegir_skirmisher"),
				(faction_set_slot, "fac_kingdom_2", slot_faction_troop_6, "trp_vaegir_archer"),
				(faction_set_slot, "fac_kingdom_2", slot_faction_troop_7, "trp_vaegir_light_cavalry"),
				(faction_set_slot, "fac_kingdom_2", slot_faction_troop_8, "trp_vaegir_horseman"),
				(faction_set_slot, "fac_kingdom_2", slot_faction_troop_9, "trp_vaegir_noble3"),
				(faction_set_slot, "fac_kingdom_2", slot_faction_troop_10, "trp_vaegir_noble4"),
				
				(faction_set_slot, "fac_kingdom_3", slot_faction_troop_1, "trp_khergit_skirmisher"),
				(faction_set_slot, "fac_kingdom_3", slot_faction_troop_2, "trp_khergit_skirmisher"),
				(faction_set_slot, "fac_kingdom_3", slot_faction_troop_3, "trp_khergit_horseman"),
				(faction_set_slot, "fac_kingdom_3", slot_faction_troop_4, "trp_khergit_dune_rider"),
				(faction_set_slot, "fac_kingdom_3", slot_faction_troop_5, "trp_khergit_horse_archer"),
				(faction_set_slot, "fac_kingdom_3", slot_faction_troop_6, "trp_khergit_archer"),
				(faction_set_slot, "fac_kingdom_3", slot_faction_troop_7, "trp_khergit_steppe_fighter"),
				(faction_set_slot, "fac_kingdom_3", slot_faction_troop_8, "trp_khergit_soldier"),
				(faction_set_slot, "fac_kingdom_3", slot_faction_troop_9, "trp_khergit_noble3"),
				(faction_set_slot, "fac_kingdom_3", slot_faction_troop_10, "trp_khergit_noble4"),
				
				(faction_set_slot, "fac_kingdom_4", slot_faction_troop_1, "trp_nord_footman"),
				(faction_set_slot, "fac_kingdom_4", slot_faction_troop_2, "trp_nord_footman"),
				(faction_set_slot, "fac_kingdom_4", slot_faction_troop_3, "trp_nord_trained_footman"),
				(faction_set_slot, "fac_kingdom_4", slot_faction_troop_4, "trp_nord_warrior"),
				(faction_set_slot, "fac_kingdom_4", slot_faction_troop_5, "trp_nord_huntsman"),
				(faction_set_slot, "fac_kingdom_4", slot_faction_troop_6, "trp_nord_archer"),
				(faction_set_slot, "fac_kingdom_4", slot_faction_troop_7, "trp_nord_trained_footman"),
				(faction_set_slot, "fac_kingdom_4", slot_faction_troop_8, "trp_nord_veteran"),
				(faction_set_slot, "fac_kingdom_4", slot_faction_troop_9, "trp_nord_noble3"),
				(faction_set_slot, "fac_kingdom_4", slot_faction_troop_10, "trp_nord_noble4"),
				
				(faction_set_slot, "fac_kingdom_5", slot_faction_troop_1, "trp_rhodok_spearman"),
				(faction_set_slot, "fac_kingdom_5", slot_faction_troop_2, "trp_rhodok_crossbowman"),
				(faction_set_slot, "fac_kingdom_5", slot_faction_troop_3, "trp_rhodok_trained_spearman"),
				(faction_set_slot, "fac_kingdom_5", slot_faction_troop_4, "trp_rhodok_veteran_spearman"),
				(faction_set_slot, "fac_kingdom_5", slot_faction_troop_5, "trp_rhodok_trained_crossbowman"),
				(faction_set_slot, "fac_kingdom_5", slot_faction_troop_6, "trp_rhodok_veteran_crossbowman"),
				(faction_set_slot, "fac_kingdom_5", slot_faction_troop_7, "trp_rhodok_trained_swordman"),
				(faction_set_slot, "fac_kingdom_5", slot_faction_troop_8, "trp_rhodok_mounted_trained_skirmisher"),
				(faction_set_slot, "fac_kingdom_5", slot_faction_troop_9, "trp_rhodok_noble3"),
				(faction_set_slot, "fac_kingdom_5", slot_faction_troop_10, "trp_rhodok_noble4"),
				
				(faction_set_slot, "fac_kingdom_6", slot_faction_troop_1, "trp_sarranid_footman"),
				(faction_set_slot, "fac_kingdom_6", slot_faction_troop_2, "trp_sarranid_footman"),
				(faction_set_slot, "fac_kingdom_6", slot_faction_troop_3, "trp_sarranid_veteran_footman"),
				(faction_set_slot, "fac_kingdom_6", slot_faction_troop_4, "trp_sarranid_infantry"),
				(faction_set_slot, "fac_kingdom_6", slot_faction_troop_5, "trp_sarranid_skirmisher"),
				(faction_set_slot, "fac_kingdom_6", slot_faction_troop_6, "trp_sarranid_archer"),
				(faction_set_slot, "fac_kingdom_6", slot_faction_troop_7, "trp_sarranid_cavalry"),
				(faction_set_slot, "fac_kingdom_6", slot_faction_troop_8, "trp_sarranid_horseman"),
				(faction_set_slot, "fac_kingdom_6", slot_faction_troop_9, "trp_sarranid_noble3"),
				(faction_set_slot, "fac_kingdom_6", slot_faction_troop_10, "trp_sarranid_noble4"),
				
				(faction_set_slot, "fac_kingdom_9", slot_faction_troop_1, "trp_mountain_bandit"),
				(faction_set_slot, "fac_kingdom_9", slot_faction_troop_2, "trp_forest_bandit"),
				(faction_set_slot, "fac_kingdom_9", slot_faction_troop_3, "trp_taiga_bandit"),
				(faction_set_slot, "fac_kingdom_9", slot_faction_troop_4, "trp_sea_raider"),
				(faction_set_slot, "fac_kingdom_9", slot_faction_troop_5, "trp_mountain_raider"),
				(faction_set_slot, "fac_kingdom_9", slot_faction_troop_6, "trp_forest_hunter"),
				(faction_set_slot, "fac_kingdom_9", slot_faction_troop_7, "trp_desert_bandit"),
				(faction_set_slot, "fac_kingdom_9", slot_faction_troop_8, "trp_steppe_bandit"),
				(faction_set_slot, "fac_kingdom_9", slot_faction_troop_9, "trp_skull_crusher"),
				(faction_set_slot, "fac_kingdom_9", slot_faction_troop_10, "trp_assassin"),
				
				(faction_set_slot, "fac_kingdom_10", slot_faction_troop_1, "trp_watchman"),
				(faction_set_slot, "fac_kingdom_10", slot_faction_troop_2, "trp_watchman"),
				(faction_set_slot, "fac_kingdom_10", slot_faction_troop_3, "trp_town_watch"),
				(faction_set_slot, "fac_kingdom_10", slot_faction_troop_4, "trp_mercenary_swordsman"),
				(faction_set_slot, "fac_kingdom_10", slot_faction_troop_5, "trp_mercenary_fieldman"),
				(faction_set_slot, "fac_kingdom_10", slot_faction_troop_6, "trp_mercenary_horse_archer"),
				(faction_set_slot, "fac_kingdom_10", slot_faction_troop_7, "trp_caravan_guard"),
				(faction_set_slot, "fac_kingdom_10", slot_faction_troop_8, "trp_mercenary_horseman"),
				(faction_set_slot, "fac_kingdom_10", slot_faction_troop_9, "trp_player_noble3"),
				(faction_set_slot, "fac_kingdom_10", slot_faction_troop_10, "trp_player_noble4"),
			]),
		...
		(0, 0, 0, [],
			[
				(try_begin),
					...
					(val_add, "$g_team_1_faction", 1),
					(try_begin),
						(gt, "$g_team_1_faction", "fac_kingdom_10"),
						(assign, "$g_team_1_faction", "fac_kingdom_1"),
					(try_end),
					(str_store_faction_name, s20, "$g_team_1_faction"),
					(display_message, "@Team 1 is now {s20}!"),
				(try_end),
				(try_begin),
					(key_clicked, key_right),
					(val_add, "$g_team_2_faction", 1),
					(try_begin),
						(gt, "$g_team_2_faction", "fac_kingdom_10"),
						(assign, "$g_team_2_faction", "fac_kingdom_1"),
					(try_end),
					(str_store_faction_name, s20, "$g_team_2_faction"),
					(display_message, "@Team 2 is now {s20}!"),
				(try_end),
			]),
		...
		(ti_on_agent_spawn, 0, 0, [],
			[
				...
				(call_script, "script_reassign_agent_division", ":agent_no"),
			]),
		...
		(0, 0, ti_once, [],
			[
				...
				# (add_visitors_to_current_scene, 1, "trp_swadian_militia", 25),
				# (add_visitors_to_current_scene, 1, "trp_swadian_footman", 12),
				# (add_visitors_to_current_scene, 1, "trp_swadian_skirmisher", 12),
				# (add_visitors_to_current_scene, 1, "trp_swadian_cavalryman", 9),
				(add_visitors_to_current_scene, 1, "trp_swadian_crossbowman", 5),
				(add_visitors_to_current_scene, 1, "trp_swadian_infantry", 5),
				(add_visitors_to_current_scene, 1, "trp_swadian_man_at_arms", 3),
				(add_visitors_to_current_scene, 1, "trp_swadian_noble4", 1),
				
				# (add_visitors_to_current_scene, 2, "trp_vaegir_scout", 13),
				# (add_visitors_to_current_scene, 2, "trp_vaegir_footman", 12),
				# (add_visitors_to_current_scene, 2, "trp_vaegir_veteran", 12),
				# (add_visitors_to_current_scene, 2, "trp_vaegir_skirmisher", 12),
				# (add_visitors_to_current_scene, 2, "trp_vaegir_light_cavalry", 9),
				(add_visitors_to_current_scene, 2, "trp_vaegir_archer", 5),
				(add_visitors_to_current_scene, 2, "trp_vaegir_infantry", 5),
				(add_visitors_to_current_scene, 2, "trp_vaegir_horseman", 3),
				(add_visitors_to_current_scene, 2, "trp_vaegir_noble4", 1),
				
				(try_for_range, ":team", 1, 3),
					(call_script, "script_find_new_leader", ":team"),
					(gt, reg0, 0),
					(team_set_leader, ":team", reg0),
				(try_end),
				
				(call_script, "script_battle_tactic_init_expanded"),
			]),
			
		(5, 0, 0, [],
			[
				(call_script, "script_battle_tactic_apply_expanded"),
			]),
			
		(15, 0, 0, [], # Handles reinforcements for both sides
			[
				(assign, ":num_team1", 0),
				(assign, ":num_team2", 0),
				(try_for_agents, ":agent_no"),
				...
				(try_begin),
					(lt, ":num_team1", 50),
					(faction_get_slot, ":troop1", "$g_team_1_faction", slot_faction_troop_1),
					(faction_get_slot, ":troop2", "$g_team_1_faction", slot_faction_troop_2),
					(faction_get_slot, ":troop3", "$g_team_1_faction", slot_faction_troop_3),
					(faction_get_slot, ":troop4", "$g_team_1_faction", slot_faction_troop_4),
					(faction_get_slot, ":troop5", "$g_team_1_faction", slot_faction_troop_5),
					(faction_get_slot, ":troop6", "$g_team_1_faction", slot_faction_troop_6),
					(faction_get_slot, ":troop7", "$g_team_1_faction", slot_faction_troop_7),
					(faction_get_slot, ":troop8", "$g_team_1_faction", slot_faction_troop_8),
					(faction_get_slot, ":troop9", "$g_team_1_faction", slot_faction_troop_9),
					(faction_get_slot, ":troop10", "$g_team_1_faction", slot_faction_troop_10),
					
					(add_visitors_to_current_scene, 1, ":troop1", 20),	# Tier 2 melee
					(add_visitors_to_current_scene, 1, ":troop2", 20),	# Tier 2 ranged
					(add_visitors_to_current_scene, 1, ":troop3", 15),	# Tier 3 melee
					(add_visitors_to_current_scene, 1, ":troop4", 7),	# Tier 4 melee
					(add_visitors_to_current_scene, 1, ":troop5", 15),	# Tier 3 ranged
					(add_visitors_to_current_scene, 1, ":troop6", 7),	# Tier 4 ranged
					(add_visitors_to_current_scene, 1, ":troop7", 12),	# Tier 3 cavalry
					(add_visitors_to_current_scene, 1, ":troop8", 5),	# Tier 4 cavalry
					(add_visitors_to_current_scene, 1, ":troop9", 3),	# Tier 3 noble
					(add_visitors_to_current_scene, 1, ":troop10", 1),	# Tier 4 noble
					(try_for_range, ":div", 0, 4),
						(val_add, ":div", slot_team_division_0_state),
						(team_set_slot, 1, ":div", stds_preparation),
					(try_end),
					(try_for_range, ":div", 4, 8),
						(val_add, ":div", slot_team_division_0_state),
						(team_set_slot, 1, ":div", stds_preparation),
					(try_end),
					(team_set_slot, 1, slot_team_battle_tactic, bt_regroup),
					(display_message, "@Team 1 Reinforcements!"),
				(try_end),
				(try_begin),
					(lt, ":num_team2", 50),
					(faction_get_slot, ":troop1", "$g_team_2_faction", slot_faction_troop_1),
					(faction_get_slot, ":troop2", "$g_team_2_faction", slot_faction_troop_2),
					(faction_get_slot, ":troop3", "$g_team_2_faction", slot_faction_troop_3),
					(faction_get_slot, ":troop4", "$g_team_2_faction", slot_faction_troop_4),
					(faction_get_slot, ":troop5", "$g_team_2_faction", slot_faction_troop_5),
					(faction_get_slot, ":troop6", "$g_team_2_faction", slot_faction_troop_6),
					(faction_get_slot, ":troop7", "$g_team_2_faction", slot_faction_troop_7),
					(faction_get_slot, ":troop8", "$g_team_2_faction", slot_faction_troop_8),
					(faction_get_slot, ":troop9", "$g_team_2_faction", slot_faction_troop_9),
					(faction_get_slot, ":troop10", "$g_team_2_faction", slot_faction_troop_10),
					
					(add_visitors_to_current_scene, 2, ":troop1", 20),
					(add_visitors_to_current_scene, 2, ":troop2", 20),
					(add_visitors_to_current_scene, 2, ":troop3", 15),
					(add_visitors_to_current_scene, 2, ":troop4", 7),
					(add_visitors_to_current_scene, 2, ":troop5", 15),
					(add_visitors_to_current_scene, 2, ":troop6", 7),
					(add_visitors_to_current_scene, 2, ":troop7", 12),
					(add_visitors_to_current_scene, 2, ":troop8", 5),
					(add_visitors_to_current_scene, 2, ":troop9", 3),
					(add_visitors_to_current_scene, 2, ":troop10", 1),
					(try_for_range, ":div", 0, 4),
						(val_add, ":div", slot_team_division_0_state),
						(team_set_slot, 2, ":div", stds_preparation),
					(try_end),
					(try_for_range, ":div", 4, 8),
						(val_add, ":div", slot_team_division_0_state),
						(team_set_slot, 2, ":div", stds_preparation),
					(try_end),
					(team_set_slot, 2, slot_team_battle_tactic, bt_regroup),
					(display_message, "@Team 2 Reinforcements!"),
				(try_end),
			]),
		...
		(10, 0, 0, # Reassign new leader if old one is unable to lead
			...
			[
				(try_for_range, ":team", 0, 3),
					...
					(this_or_next|lt, ":leader", 0),
					(neg|agent_is_alive, ":leader"),
					(call_script, "script_find_new_leader", ":team"),
					(try_begin),
						(gt, reg0, -1),
						(team_set_leader, ":team", reg0),
						(display_message, "@Team found a new leader!"),
					(else_try),
						(try_begin),
							(eq, ":team", 0),
							# do nothing?
						(else_try),
							(eq, ":team", 1),
							# (assign, "$ai_team_1_battle_tactic", bt_none), # No more leader, chances are that they will start having weird behaviors
							# (team_set_slot, 1, slot_team_battle_tactic, bt_none),
							(display_message, "@No leader found for team 1!"),
						(else_try),
							(eq, ":team", 2),
							# (assign, "$ai_team_2_battle_tactic", bt_none), # Such as changing tactic to charge, or try to regroup even if it's dangerous
							# (team_set_slot, 1, slot_team_battle_tactic, bt_none),
							(display_message, "@No leader found for team 2!"),
						(try_end),
					(try_end),
				(try_end),
			]),
		...
		(ti_on_agent_dismount, 0, 0,
			...
			[
				...
				(agent_is_human, ":agent_id"),
				# (try_begin),
					# (agent_get_division, ":div", ":agent_id"),
					# (this_or_next|eq, ":div", 2),
					# (eq, ":div", 3),
					(call_script, "script_reassign_agent_division", ":agent_id"),
				# (try_end),
			]),
			
		(ti_on_item_unwielded, 0, 0,
			[
				(store_trigger_param, ":item_id", 2),
		...
		(10, 0, 0, # Handles retreats and group combinations
			[],
			[
				# For now just 2 teams are counted
				(try_for_range, ":team", 1, 3),
					(assign, ":num_inf", 0),
					(assign, ":num_rang", 0),
					(assign, ":num_cav", 0),
					...
					# (team_get_slot, ":div0_state", ":team", slot_team_division_0_state),
					# (team_get_slot, ":div1_state", ":team", slot_team_division_1_state),
					# (team_get_slot, ":div2_state", ":team", slot_team_division_2_state),
					# (team_get_slot, ":div3_state", ":team", slot_team_division_3_state),
					
					# (assign, ":accum_pos_x_inf", 0),
					# (assign, ":accum_pos_y_inf", 0),
					# (assign, ":accum_pos_z_inf", 0),
					# (assign, ":accum_pos_x_rang", 0),
					# (assign, ":accum_pos_y_rang", 0),
					# (assign, ":accum_pos_z_rang", 0),
					# (assign, ":accum_pos_x_cav", 0),
					# (assign, ":accum_pos_y_cav", 0),
					# (assign, ":accum_pos_z_cav", 0),
					# (assign, ":accum_pos_x_aux", 0),
					# (assign, ":accum_pos_y_aux", 0),
					# (assign, ":accum_pos_z_aux", 0),
					...
					(try_for_agents, ":agent_no"),
						...
						(try_begin),
							...
							(val_add, ":num_inf", 1),
							# (try_begin),
								# (ge, ":div0_state", stds_reform),
								# (agent_get_position, pos62, ":agent_no"),

								# (position_get_x, ":x", pos62),
								# (position_get_y, ":y", pos62),
								# (position_get_y, ":z", pos62),

								# (val_add, ":accum_pos_x_inf", ":x"),
								# (val_add, ":accum_pos_y_inf", ":y"),
								# (val_add, ":accum_pos_z_inf", ":z"),
							# (try_end),
						(else_try),
							(eq, ":division", 1), # Archer
							(val_add, ":num_rang", 1),
							# (try_begin),
								# (ge, ":div1_state", stds_reform),
								# (agent_get_position, pos62, ":agent_no"),
								...
								# (val_add, ":accum_pos_x_rang", ":x"),
								# (val_add, ":accum_pos_y_rang", ":y"),
								# (val_add, ":accum_pos_z_rang", ":z"),
							# (try_end),
						(else_try),
							(eq, ":division", 2), # Cavalry
							(val_add, ":num_cav", 1),
							# (try_begin),
								# (ge, ":div2_state", stds_reform),
								# (agent_get_position, pos62, ":agent_no"),
								...
								# (val_add, ":accum_pos_x_cav", ":x"),
								# (val_add, ":accum_pos_y_cav", ":y"),
								# (val_add, ":accum_pos_z_cav", ":z"),
							# (try_end),
						(else_try),
							(eq, ":division", 3), # Auxiliary
							(val_add, ":num_aux", 1),
							# (try_begin),
								# (ge, ":div3_state", stds_reform),
								# (agent_get_position, pos62, ":agent_no"),
								...
								# (val_add, ":accum_pos_x_aux", ":x"),
								# (val_add, ":accum_pos_y_aux", ":y"),
								# (val_add, ":accum_pos_z_aux", ":z"),
							# (try_end),
						(try_end),
					(try_end),
					...
					# (try_begin),
						# (gt, ":num_inf", 0),
						# (store_div, ":average_x", ":accum_pos_x_inf", ":num_inf"),
						# (store_div, ":average_y", ":accum_pos_y_inf", ":num_inf"),
						# (store_div, ":average_z", ":accum_pos_z_inf", ":num_inf"),
						
						# (init_position, pos25),
						# (position_set_x, pos25, ":average_x"),
						# (position_set_y, pos25, ":average_y"),
						# (position_set_z, pos25, ":average_z"),
						
						# (team_get_order_position, pos26, ":team", 0),
						...
						# (try_begin),
							# (team_slot_eq, ":team", slot_team_battle_tactic, bt_regroup),
					...
					# (try_begin),
						# (gt, ":num_rang", 0),
						# (store_div, ":average_x", ":accum_pos_x_rang", ":num_rang"),
						# (store_div, ":average_y", ":accum_pos_y_rang", ":num_rang"),
						# (store_div, ":average_z", ":accum_pos_z_rang", ":num_rang"),
						...
						# (team_get_order_position, pos26, ":team", 1),
					...
					# (try_begin),
						# (gt, ":num_cav", 0),
						# (store_div, ":average_x", ":accum_pos_x_cav", ":num_cav"),
						# (store_div, ":average_y", ":accum_pos_y_cav", ":num_cav"),
						# (store_div, ":average_z", ":accum_pos_z_cav", ":num_cav"),
						...
						# (team_get_order_position, pos26, ":team", 2),
					...
					# (try_begin),
						# (gt, ":num_aux", 0),
						# (store_div, ":average_x", ":accum_pos_x_aux", ":num_aux"),
						# (store_div, ":average_y", ":accum_pos_y_aux", ":num_aux"),
						# (store_div, ":average_z", ":accum_pos_z_aux", ":num_aux"),
						...
						# (team_get_order_position, pos26, ":team", 3),
					...
					# (call_script, "script_assign_combat_division", ":num_inf", ":num_rang", ":num_cav", ":num_aux", ":team"),
					
					(team_set_slot, ":team", slot_team_num_agent_in_div0, ":num_inf"),
					(team_set_slot, ":team", slot_team_num_agent_in_div1, ":num_rang"),
					(team_set_slot, ":team", slot_team_num_agent_in_div2, ":num_cav"),
					(team_set_slot, ":team", slot_team_num_agent_in_div3, ":num_aux"),
