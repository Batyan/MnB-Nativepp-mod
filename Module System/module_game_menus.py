    ...
    [##diplomacy start+ Replace "join" with "Join" in the following
      ("town_1",[(eq, "$current_startup_quest_phase", 0),],"Join a caravan to Praven, in the Kingdom of Swadia.",
       [
		 ...
		 (try_begin),
		   (eq, "$cloth_fac", 1),
		   (troop_add_item, "trp_player","itm_red_tunic",0),
		 (try_end),
		 (try_begin),
		   (eq, "$shield_fac", 1),
		   (troop_add_item, "trp_player","itm_tab_shield_heater_a",imod_battered),
		 (try_end),
		 (try_begin),
		   (eq, "$sword_fac", 1),
...
#         (change_screen_return),
      ...
      ("town_2",[(eq, "$current_startup_quest_phase", 0),],"Join a caravan to Reyvadin, in the Kingdom of the Vaegirs.",
       [
		 ...
		 (try_begin),
		   (eq, "$shield_fac", 1),
		   (troop_add_item, "trp_player","itm_tab_shield_kite_a",imod_battered),
		 (try_end),
		 (try_begin),
		   (eq, "$sword_fac", 1),
...
#         (change_screen_return),
      ...
      ("town_4",[(eq, "$current_startup_quest_phase", 0),],"Take a ship to Sargoth, in the Kingdom of the Nords.",
       [
		 ...
		 (try_begin),
		   (eq, "$sword_fac", 1),
		   (troop_add_item, "trp_player","itm_sword_viking_1",imod_rusty),
		 (else_try),
		   (eq, "$sword_fac", 2),
		   (troop_add_item, "trp_player","itm_one_handed_war_axe_a",imod_rusty),
		 (try_end),
		 (troop_equip_items, "trp_player"),
		 (set_show_messages, 1),
		 (jump_to_menu, "mnu_start_phase_2_5"),
...
#         (change_screen_return),
      ...
      ("town_5",[(eq, "$current_startup_quest_phase", 0),],"Take a ship to Jelkala, in the Kingdom of the Rhodoks.",
       [
		 ...
		 (try_begin),
		   (eq, "$shield_fac", 1),
		   (troop_add_item, "trp_player","itm_tab_shield_pavise_a",imod_battered),
		 (try_end),
		 (try_begin),
		   (eq, "$sword_fac", 1),
		   (troop_add_item, "trp_player","itm_sword_medieval_a",imod_rusty),
...
#         (change_screen_return),
      ...
      ("town_6",[(eq, "$current_startup_quest_phase", 0),],"Join a caravan to Shariz, in the Sarranid Sultanate.",
       [
		 ...
		 (try_begin),
		   (eq, "$cloth_fac", 1),
		   (troop_add_item, "trp_player","itm_sarranid_cloth_robe",0),
		 (try_end),
		 (try_begin),
		   (eq, "$shield_fac", 1),
		 ...
		 (try_begin),
		   (eq, "$sword_fac", 1),
		   (troop_add_item, "trp_player","itm_arabian_sword_a",imod_rusty),
		 (else_try),
		   (eq, "$sword_fac", 2),
...
##    (else_try),
        ...
        (troop_add_item, "trp_player","itm_arrows",imod_bent),
        (troop_add_item, "trp_player","itm_sumpter_horse",imod_heavy),
        (troop_add_item, "trp_player","itm_dried_meat",0),
        (troop_add_item, "trp_player","itm_dried_meat",0),
        (troop_add_item, "trp_player","itm_furs",0),
...
##        (troop_add_item, "trp_player","itm_sword_medieval_a", imod_rusty),
        (troop_add_item, "trp_player","itm_hunting_crossbow", 0),
        (troop_add_item, "trp_player","itm_bolts", imod_bent),
##        (troop_add_item, "trp_player","itm_saddle_horse",imod_swaybacked),
        (troop_add_item, "trp_player","itm_smoked_fish",0),		
		(assign, "$cloth_fac", 0), 
    (else_try),
...
##  	    (troop_add_item, "trp_player","itm_club", 0),
		  (assign, "$sword_fac", 2),
		  (troop_add_item, "trp_player","itm_sumpter_horse", 0),
          (troop_add_item, "trp_player","itm_blue_dress",imod_sturdy),
		(else_try),
		  (eq, ":random", 2),
		  (troop_add_item, "trp_player","itm_dagger", 0),
		  (troop_add_item, "trp_player","itm_throwing_knives", 0),
          (troop_add_item, "trp_player","itm_dress",imod_sturdy),
		(else_try),
		  (troop_add_item, "trp_player","itm_sickle", 0),
...
#             (start_presentation, "prsnt_custom_banner"),
  ...
  ("morale_report",0,
   ...
   [
	 (try_begin),
	   ...
	   (try_begin),
	     (gt, reg5, 90),
	     (assign, ":string", "str_stability_90"),
	   (else_try),
	     (gt, reg5, 75),
	     (assign, ":string", "str_stability_75"),
	   (else_try),
	     (gt, reg5, 60),
	     (assign, ":string", "str_stability_60"),
	   (else_try),
	     (gt, reg5, 45),
	     (assign, ":string", "str_stability_45"),
	   (else_try),
	     (gt, reg5, 30),
	     (assign, ":string", "str_stability_30"),
	   (else_try),
	     (gt, reg5, 15),
	     (assign, ":string", "str_stability_15"),
	   (else_try),
	     (gt, reg5, 0),
	     (assign, ":string", "str_stability_0"),
	   (else_try),
	     (assign, ":string", "str_stability_negative"),
	   (try_end),
	   (str_store_string, s8, ":string"),
	   
	   (party_get_morale, reg7, "p_main_party"),
       
       (str_store_string, s1, "str_stability_is_s8_current_party_morale_is_reg5_current_party_morale_modifiers_are__base_morale__50_party_size_s2reg1_leadership_s3reg2_food_variety_s4reg3s5s6_recent_events_s7reg4_total__reg5___"),
  ...
  ("party_size_report",0,
   ...
   [(call_script, "script_game_get_party_companion_limit"),
    ...
    (assign, reg3, ":renown"),
    (str_store_string, s1, "@Current party size limit is {reg5}.^Current party size modifiers are:^^Leadership: {s2}{reg1}^Charisma: {s3}{reg2}^Renown: {s4}{reg3}^TOTAL:  {reg5}"),
    ],
    [
      ("continue",[],"Continue...",
       [(jump_to_menu, "mnu_reports"),
...
##diplomacy end
	  ...
	  ("camp_test",[(eq, "$test", 1),], "Test Menu",
	   [(jump_to_menu, "mnu_camp_test"),
	    ],
	  ),
      ]
  ),
  ("camp_test", 0, 
   "Testing room. Do not enter.",
   "none",
   [],
   [
	  ("town_1",[],"Go to intro battle scene",
	    [(set_jump_mission, "mt_intro_mission"),
		 (jump_to_scene, "scn_intro_battle_plain"),
		 (change_screen_mission),       
	    ]
	  ),
	  
	  ("plain_field_battle",[], "Test field battles in plain",
		[
			(set_jump_mission, "mt_field_battle_test"),
			(jump_to_scene, "scn_test_field_battle"),
			(change_screen_mission),  
		]),
  ...
  ("camp_action",0,
   ...
   [
    ...
    [
      ...
      ("action_kingdom_managment",[(eq, "$players_kingdom_name_set", 1),],"Modify your kingdom.", ##new 0.751
       [(jump_to_menu, "mnu_kingdom_managment"),
        ]
       ),
    ...
    [
      ("camp_recruit_prisoners_accept",[(gt, "$g_prisoner_recruit_troop_id", 0)],"Take them.",
       [(remove_troops_from_prisoners, "$g_prisoner_recruit_troop_id", "$g_prisoner_recruit_size"),
        ...
        (store_mul, ":morale_change", -1, "$g_prisoner_recruit_size"),
 ...
 Try as you might, it soon gives you a headache, and you're forced to give up the attempt."),
  ...
  ("kingdom_managment",0, ##new 0.751
   "Choose an action",
   "none",
   [], 
   [
     ("culture",[],"Change the culture of your kingdom.",
       [(jump_to_menu, "mnu_change_culture"),
        ]
       ),
	 ("color",[],"Change your kingdom color.",
  ...
  ("change_culture",0,
   "Choose your kingdom's culture:",
   "none",
   [], 
   [
     ("culture_1",[],"Swadian.",
       [(faction_set_slot, "fac_player_supporters_faction", slot_faction_culture, "fac_culture_1"),
	    (faction_set_slot, "fac_player_faction", slot_faction_culture, "fac_culture_1"),
		(call_script, "script_initialize_player_faction_troop_type"), ##new 0.751
	 ...
	 ("culture_2",[],"Vaegir.",
       [(faction_set_slot, "fac_player_supporters_faction", slot_faction_culture, "fac_culture_2"),
	    (faction_set_slot, "fac_player_faction", slot_faction_culture, "fac_culture_2"),
		(call_script, "script_initialize_player_faction_troop_type"), ##new 0.751
	 ...
	 ("culture_3",[],"Khergit.",
       [(faction_set_slot, "fac_player_supporters_faction", slot_faction_culture, "fac_culture_3"),
	    (faction_set_slot, "fac_player_faction", slot_faction_culture, "fac_culture_3"),
		(call_script, "script_initialize_player_faction_troop_type"),##new 0.751
	 ...
	 ("culture_4",[],"Nord.",
       [(faction_set_slot, "fac_player_supporters_faction", slot_faction_culture, "fac_culture_4"),
	    (faction_set_slot, "fac_player_faction", slot_faction_culture, "fac_culture_4"),
		(call_script, "script_initialize_player_faction_troop_type"),##new 0.751
	 ...
	 ("culture_5",[],"Rhodok.",
       [(faction_set_slot, "fac_player_supporters_faction", slot_faction_culture, "fac_culture_5"),
	    (faction_set_slot, "fac_player_faction", slot_faction_culture, "fac_culture_5"),
		(call_script, "script_initialize_player_faction_troop_type"),##new 0.751
	 ...
	 ("culture_6",[],"Sarranid.",
       [(faction_set_slot, "fac_player_supporters_faction", slot_faction_culture, "fac_culture_6"),
	    (faction_set_slot, "fac_player_faction", slot_faction_culture, "fac_culture_6"),
		(call_script, "script_initialize_player_faction_troop_type"),##new 0.751
	 ...
	 ("culture_8",[],"Freeriders.",
       [(faction_set_slot, "fac_player_supporters_faction", slot_faction_culture, "fac_culture_8"),
		...
		(call_script, "script_initialize_player_faction_troop_type"),##new 0.751
		(assign, "$g_player_culture", "fac_kingdom_10"),
        (jump_to_menu, "mnu_camp_action"),
        ]
       ),
	 ("culture_9",[],"Khergit Tribes.",
       [(faction_set_slot, "fac_player_supporters_faction", slot_faction_culture, "fac_culture_9"),
	    (faction_set_slot, "fac_player_faction", slot_faction_culture, "fac_culture_9"),
		(call_script, "script_initialize_player_faction_troop_type"),##new 0.751
  ...
  ("change_color",0,
   "Change your kingdom color",
   "none",
   [], 
   [
     ("color_1",[],"White.",
       [(faction_set_color, "fac_player_supporters_faction", 0xF0F0F0),
        (jump_to_menu, "mnu_camp_action"),
        ]
       ),
	 ("color_2",[],"Black.",
       [(faction_set_color, "fac_player_supporters_faction", 0x0F0F0F),
        (jump_to_menu, "mnu_camp_action"),
        ]
       ),
	 ("color_3",[],"Blue.",
       [(faction_set_color, "fac_player_supporters_faction", 0x0000FF),
        (jump_to_menu, "mnu_camp_action"),
        ]
       ),
	 ("color_4",[],"Red.",
	 ...
	 ("color_5",[],"Green.",
       [(faction_set_color, "fac_player_supporters_faction", 0x00FF00),
        (jump_to_menu, "mnu_camp_action"),
        ]
       ),
	 ("more",[],"More Colors.",
       [(jump_to_menu, "mnu_change_color_a"),
        ]
       ),
	]),
	
  ("change_color_a",0,
   "Choose your kingdom color",
   "none",
   [], 
   [
     ("color_1",[],"Grey.",
       [(faction_set_color, "fac_player_supporters_faction", 0x808080),
        (jump_to_menu, "mnu_camp_action"),
        ]
       ),
	 ("color_2",[],"Brown.",
       [(faction_set_color, "fac_player_supporters_faction", 0x582900),
        (jump_to_menu, "mnu_camp_action"),
        ]
       ),
	 ("color_3",[],"Cyan.",
       [(faction_set_color, "fac_player_supporters_faction", 0x00FFFF),
        (jump_to_menu, "mnu_camp_action"),
        ]
       ),
	 ("color_4",[],"Yellow.",
       [(faction_set_color, "fac_player_supporters_faction", 0xFFFF00),
        (jump_to_menu, "mnu_camp_action"),
        ]
       ),
	 ("color_5",[],"Magenta.",
       [(faction_set_color, "fac_player_supporters_faction", 0xFF00FF),
        (jump_to_menu, "mnu_camp_action"),
        ]
       ),
	 ("more",[],"More Colors.",
...
## PreBattle Orders & Deployment End
      ...
      [
	   ...
	   (else_try),
	     (eq, "$do_not_command_troops", 1),
		 (str_store_string, s20, "@don't control your troops"),
	   (else_try),
	     (str_store_string, s20, "@control all troops if possible"),
	   (try_end),
	   ],
	   
      "Change control of your troops (current : You {s20}).",
      [(val_add, "$do_not_command_troops", 1),
	   (val_mod, "$do_not_command_troops", 3),
      ]),
...
###Troop commentary changes end          
  ...
  (
    ...
    [
      ...
      (val_add, ":enemy_party_strength", 1),
	  (val_div, ":total_player_and_followers_strength", 5),
      (val_add, ":total_player_and_followers_strength", 1),

	  (val_div, ":player_party_defense", 5),
      (val_max, ":player_party_defense", 1),
      (val_div, ":enemy_party_defense", 5),
      (val_add, ":enemy_party_defense", 1),
	  (val_div, ":total_player_and_followers_defense", 5),
      (val_add, ":total_player_and_followers_defense", 1),
	  ...
	  (store_mul, ":defense_contribution_of_player", ":player_party_defense", 100),
      (val_div, ":defense_contribution_of_player", ":total_player_and_followers_defense"),
	  
	  (try_begin), # We reduce ennemy's strength to a level closer to the player
	    (gt, ":enemy_party_strength", ":total_player_and_followers_strength"),
	    (val_add, ":enemy_party_strength", ":total_player_and_followers_strength"),
	    (val_div, ":enemy_party_strength", 2),
	  (else_try), # Unless player has more strength
	    (store_add, ":ally_strength", ":total_player_and_followers_strength", ":enemy_party_strength"),
	    (val_div, ":ally_strength", 2),
	  (try_end),
	  
	  (try_begin), # We do the same for defense, except that we increase defender's defense
	    (gt, ":enemy_party_defense", ":total_player_and_followers_defense"),
	    (store_add, ":ally_defense", ":total_player_and_followers_defense", ":enemy_party_defense"),
	    (val_div, ":ally_defense", 2),
		
		(store_mul, ":player_party_defense", ":ally_defense", ":defense_contribution_of_player"),
		(val_div, ":player_party_defense", 100),
	  (else_try),
	    (val_add, ":enemy_party_defense", ":total_player_and_followers_defense"),
	    (val_div, ":enemy_party_defense", 2),
	  (try_end),
	  ...
	  (store_div, ":player_reduce_dmg", ":player_attacked", ":player_party_defense"),
	  (val_min, ":player_reduce_dmg", 50),
	  (val_add, ":player_reduce_dmg", 50), # 50% reduction max
	  (store_mul, ":player_attack", ":player_attacked", ":player_reduce_dmg"),
	  (val_div, ":player_attack", 100),
	  (set_fixed_point_multiplier, 1),
	  (store_sqrt, ":player_attack", ":player_attack"),
      ...
      (try_begin),
		...
		(val_sub, ":ally_defense", ":player_party_defense"),
		...
		(store_div, ":ally_reduce_dmg", ":ally_attacked", ":ally_defense"),
	    (val_min, ":ally_reduce_dmg", 50),
	    (val_add, ":ally_reduce_dmg", 50), # 50% reduction max
	    (store_mul, ":ally_attack", ":ally_attacked", ":ally_reduce_dmg"),
	    (val_div, ":ally_attack", 100),
	    (set_fixed_point_multiplier, 1),
	    (store_sqrt, ":ally_attack", ":ally_attack"),
	  ...
	  (store_div, ":ennemy_reduce_dmg", ":ally_strength", ":enemy_party_defense"),
	  (val_min, ":ennemy_reduce_dmg", 50),
	  (val_add, ":ennemy_reduce_dmg", 50), # 50% reduction max
	  (store_mul, ":ennemy_attack", ":ally_strength", ":ennemy_reduce_dmg"),
	  (val_div, ":ennemy_attack", 100),
	  (set_fixed_point_multiplier, 1),
	  (store_sqrt, ":ennemy_attack", ":ennemy_attack"),
      ...
      (inflict_casualties_to_party_group, "$g_encountered_party", ":ennemy_attack", "p_temp_casualties"),
...
##            (eq, ":random_attribute", ca_intelligence),
  ...
  (
    ...
    [
		...
		(store_faction_of_party, ":faction1", "$g_encountered_party_2"),
        (store_faction_of_party, ":faction2", "$g_encountered_party"),
		(str_store_faction_name, 4, ":faction1"),
		(str_store_faction_name, 3, ":faction2"),
      ],
    [
      ("pre_join_help_attackers",[
          ...
          "Move in to help the {s2} ({s4}).",[
			  ...
			  (store_faction_of_party, ":ally_faction", "$g_ally_party"),
			  (store_relation, ":ally_relation", ":ally_faction", "fac_player_supporters_faction"),
			  (try_begin),
			    (is_between, ":ally_faction", kingdoms_begin, kingdoms_end),
				(lt, ":ally_relation", 10),
				(call_script, "script_change_player_relation_with_faction", ":ally_faction", 1),
			  (try_end),
			  (store_faction_of_party, ":ennemy_fac", "$g_enemy_party"),
			  (store_relation, ":ennemy_relation", ":ennemy_fac", "fac_player_supporters_faction"),
			  (assign, ":relation_loss", 0),
			  (try_begin),
			    (is_between, ":ennemy_fac", kingdoms_begin, kingdoms_end),
				(val_sub, ":relation_loss", 1),
				(gt, ":ennemy_relation", -10),
				(val_sub, ":relation_loss", 1),
			  (try_end),
			  (call_script, "script_change_player_relation_with_faction", ":ennemy_fac", ":relation_loss"),
      ...
      ("pre_join_help_defenders",[
          ...
          "Rush to the aid of the {s1} ({s3}).",[
              (select_enemy, 1),
              (assign, "$g_enemy_party", "$g_encountered_party_2"),
              (assign, "$g_ally_party", "$g_encountered_party"),
              # (store_faction_of_party, ":attacker_faction", "$g_encountered_party_2"),
			  # (call_script, "script_change_player_relation_with_faction", ":attacker_faction", -1),
              # (store_faction_of_party, ":defender_faction", "$g_encountered_party"),
			  # (call_script, "script_change_player_relation_with_faction", ":defender_faction", 1),
			  ...
			  (try_begin),
			    (is_between, ":ennemy_fac", kingdoms_begin, kingdoms_end),
				(gt, ":ennemy_relation", -10),
				(call_script, "script_change_player_relation_with_faction", ":ennemy_fac", -1),
			  (try_end),
              (jump_to_menu,"mnu_join_battle")]),
...
## PreBattle Orders & Deployment End
	...
	## PreBattle Orders & Deployment Begin
      ...
      [
	   ...
	   (else_try),
		 ...
		 (str_store_string, s20, "@don't control your troops"),
	   (else_try),
	     (str_store_string, s20, "@control all troops if applicable"),
	   (try_end),
	   ],
	   
      "Change the control of troops (current : You {s20}).",
      [(val_add, "$do_not_command_troops", 1),
	   (val_mod, "$do_not_command_troops", 3),
      ]),
  ...
  (
    ...
    [
	  ...
	  (store_mul, ":strength_contribution_of_player", ":player_party_strength", 100),
      (val_div, ":strength_contribution_of_player", ":friend_party_strength"),
	  
	  (store_mul, ":defense_contribution_of_player", ":player_party_defense", 100),
      (val_div, ":defense_contribution_of_player", ":friend_party_defense"),
	  
	  (try_begin), # We reduce ennemy's strength to a level closer to the player
	    (gt, ":enemy_party_strength", ":friend_party_strength"),
	    (val_add, ":enemy_party_strength", ":friend_party_strength"),
	    (val_div, ":enemy_party_strength", 2),
	  (else_try), # Unless player has more strength
	    (store_add, ":friend_party_strength", ":friend_party_strength", ":enemy_party_strength"),
	    (val_div, ":friend_party_strength", 2),
	  (try_end),
	  
	  (try_begin), # We do the same for defense, except that we increase defender's defense
	    (gt, ":enemy_party_defense", ":friend_party_defense"),
	    (store_add, ":ally_defense", ":friend_party_defense", ":enemy_party_defense"),
	    (val_div, ":ally_defense", 2),
		
		(store_mul, ":player_party_defense", ":ally_defense", ":defense_contribution_of_player"),
		(val_div, ":player_party_defense", 100),
	  (else_try),
	    (val_add, ":enemy_party_defense", ":friend_party_defense"),
	    (val_div, ":enemy_party_defense", 2),
		(assign, ":ally_defense", ":friend_party_defense"),
	  (try_end),
	  ...
	  (store_mul, ":player_attacked", ":enemy_party_strength", 100),
	  (val_div, ":player_attacked", ":strength_contribution_of_player"),
	  ...
	  (store_add, ":ally_strength", ":player_party_strength", ":friend_party_strength"),
	  ...
	  (store_mul, ":ally_attacked", ":enemy_party_strength", 100),
	  (store_sub, ":ally_contribution", 100, ":strength_contribution_of_player"),
	  (val_div, ":ally_attacked", ":ally_contribution"),
 ...
 {reg4} hours.",
   ...
   (
    ...
    [
		...
		(val_mul, ":enemy_party_strength", 150), #it was 1.5 in old version, now it is only 1.23 -- back to 1.5
		(val_div, ":enemy_party_strength", 100),
			
		(val_mul, ":enemy_party_defense", 2), #twice as hard to kill defenders, we do this to make taking castles last longer

		# (store_mul, ":ennemy_attack", ":enemy_party_strength", ":enemy_party_strength"),
		# (val_div, ":ennemy_attack", ":player_party_defense"),
		
		(try_begin), # We reduce ennemy's strength to a level closer to the player
		  (gt, ":enemy_party_strength", ":player_party_strength"),
		  (val_add, ":enemy_party_strength", ":player_party_strength"),
		  (val_div, ":enemy_party_strength", 2),
		(else_try), # Unless player has more strength
		  (store_add, ":player_party_strength", ":player_party_strength", ":enemy_party_strength"),
		  (val_div, ":player_party_strength", 2),
		(try_end),
		
		(try_begin),
		  (gt, ":enemy_party_defense", ":player_party_defense"),
		  (store_add, ":player_party_defense", ":player_party_defense", ":enemy_party_defense"),
		  (val_div, ":player_party_defense", 2),
		(else_try),
		  (val_add, ":enemy_party_defense", ":player_party_defense"),
		  (val_div, ":enemy_party_defense", 2),
		(try_end),
		
		(store_div, ":player_reduce_dmg", ":enemy_party_strength", ":player_party_defense"),
		(val_min, ":player_reduce_dmg", 50),
		(val_add, ":player_reduce_dmg", 50), # 50% reduction max
		(store_mul, ":player_attack", ":enemy_party_strength", ":player_reduce_dmg"),
		(val_div, ":player_attack", 100),
		(set_fixed_point_multiplier, 1),
		(store_sqrt, ":player_attack", ":player_attack"),
		...
		(store_div, ":ennemy_reduce_dmg", ":player_party_strength", ":enemy_party_defense"),
		(val_min, ":ennemy_reduce_dmg", 50),
		(val_add, ":ennemy_reduce_dmg", 50), # 50% reduction max
		(store_mul, ":ennemy_attack", ":player_party_strength", ":ennemy_reduce_dmg"),
		(val_div, ":ennemy_attack", 100),
		(set_fixed_point_multiplier, 1),
		(store_sqrt, ":ennemy_attack", ":ennemy_attack"),
...
##      ("call_soldiers_back",[(eq, "$no_soldiers_left", 0)],"Call your soldiers back.",[(jump_to_menu,"mnu_castle_outside")]),
   ...
   (
    ...
    [
		...
		(val_mul, ":friend_party_strength", 150), #it was 1.5 in old version, now it is only 1.23 -- back to 1.5
		(val_div, ":friend_party_strength", 100),
			
		(val_mul, ":friend_party_defense", 2), #twice as hard to kill defenders, we do this to make taking castles last longer
		
		(val_mul, ":player_party_strength", 150), #it was 1.5 in old version, now it is only 1.23 -- back to 1.5
		(val_div, ":player_party_strength", 100),
			
		(val_mul, ":player_party_defense", 2), #twice as hard to kill defenders, we do this to make taking castles last longer
...
##        (display_message, "@{!}enemy_party={reg3} ally_party={reg4}"),
		...
		(try_begin), # We reduce ennemy's strength to a level closer to the player
		  ...
		  (val_div, ":enemy_party_strength", 2),
		(else_try), # Unless player has more strength
		  (store_add, ":ally_strength", ":friend_party_strength", ":enemy_party_strength"),
		  (val_div, ":ally_strength", 2),
		(try_end),
  ...
  ##diplomacy end
    ...
    [
		...
		(try_begin),
			...
			(assign, ":damage", 40),
		(try_end),
		(troop_get_slot, ":cur_wealth", "$g_encountered_party", slot_troop_wealth), ## Test for
		(val_add, ":cur_wealth", 400),                                              ## earlier
		(troop_set_slot, ":cur_wealth", "$g_encountered_party", slot_troop_wealth), ## reinforcements
...
#           (party_add_template, "$current_town", "pt_villagers_in_raid"),
 ...
 currently building {s7}. The building will be completed after {reg8} day{reg9?s:}.:}",
    ...
    [
	  ...
	  ("center_hire_trainer_1",[(eq, reg6, 0),
                                      (this_or_next|party_slot_eq, "$g_encountered_party", slot_party_type, spt_town),
                                      (party_slot_eq, "$g_encountered_party", slot_party_type, spt_castle),
                                      (party_slot_eq, "$g_encountered_party", slot_center_has_trainer_1, 0),
                                       ],
       "Hire a trainer.",[(assign, "$g_improvement_type", slot_center_has_trainer_1),
                                  (jump_to_menu, "mnu_center_improve"),]),
								  
	  ("center_hire_trainer_2",[(eq, reg6, 0),
                                      (this_or_next|party_slot_eq, "$g_encountered_party", slot_party_type, spt_town),
                                      (party_slot_eq, "$g_encountered_party", slot_party_type, spt_castle),
                                      (party_slot_eq, "$g_encountered_party", slot_center_has_trainer_2, 0),
									  (party_slot_eq, "$g_encountered_party", slot_center_has_trainer_1, 1),
                                       ],
       "Upgrade the trainer.",[(assign, "$g_improvement_type", slot_center_has_trainer_2),
                                  (jump_to_menu, "mnu_center_improve"),]),
								  
	  ("center_hire_trainer_3",[(eq, reg6, 0),
                                      (party_slot_eq, "$g_encountered_party", slot_party_type, spt_town),
                                      (party_slot_eq, "$g_encountered_party", slot_center_has_trainer_3, 0),
									  (party_slot_eq, "$g_encountered_party", slot_center_has_trainer_2, 1),
									  (party_slot_eq, "$g_encountered_party", slot_center_has_trainer_1, 1),
                                       ],
       "Upgrade even more the trainer.",[(assign, "$g_improvement_type", slot_center_has_trainer_3),
                                  (jump_to_menu, "mnu_center_improve"),]),
								
	  ("center_build_recruit_camp",[(eq, reg6, 0),
                                      (this_or_next|party_slot_eq, "$g_encountered_party", slot_party_type, spt_town),
                                      (party_slot_eq, "$g_encountered_party", slot_party_type, spt_castle),
									  (party_slot_eq, "$g_encountered_party", slot_center_has_recruitment_camp, 0),
                                       ],
       "Build a recruitment camp.",[(assign, "$g_improvement_type", slot_center_has_recruitment_camp),
                                  (jump_to_menu, "mnu_center_improve"),]),
	
	  ## bank_begin
	  ("center_build_bank",[(eq, reg6, 0),
                                      (party_slot_eq, "$g_encountered_party", slot_party_type, spt_town),
									  (party_slot_eq, "$g_encountered_party", slot_center_has_bank, 0),
                                       ],
       "Build a bank.",[(assign, "$g_improvement_type", slot_center_has_bank),
                                  (jump_to_menu, "mnu_center_improve"),]),
	  ## bank_end
	  
	  
	  ("hirer_conditions",
      [(party_slot_eq, "$current_town", slot_center_has_recruitment_camp, 1),
      ],
      "Set the maximum number of troops for the center",
      [
        (jump_to_menu, "mnu_center_hire"),
      ]),
...
#					(assign, "$g_last_assassination_attempt_perpetrator", ":lord"),
			 ...
			 (try_begin), # smith will apear in taverns
               (party_get_slot, ":smith", "$current_town", slot_center_smith),
               (gt, ":smith", 0),
               
               (assign, reg0, ":smith"),
               (assign, reg1, "$current_town"),
			   
               (set_visitor, ":cur_entry", ":smith"),
               (val_add, ":cur_entry", 1),
			 (try_end),
       ...
       [
         (try_begin),
           ...
           (try_for_range, ":troop_iterator", 0, ":num_stacks"),
             ...
             (neg|troop_is_hero, ":cur_troop_id"),
			 (neg|is_between, ":cur_troop_id", noble_troops_begin, noble_troops_end),
...
##########################
    ("recruit_nobles",[(party_slot_eq,"$current_town",slot_party_type, spt_town),
	                   (call_script, "script_cf_town_recruit_nobles_cond"),],
	 "Recruit Nobles.",
	 [
	   (jump_to_menu, "mnu_town_recruit_nobles"),
	  ]),
##########################
## recruit nobles end   ##
##########################
...
## recruit nobles start ##
##########################
  ("town_recruit_nobles",0,
  "These nobles are the best warriors of all Calradia and the most expensive, {reg5?{s20}:but none of them is} willing to follow you.",
  "none",
  [  (party_get_slot, ":volunteer_amount", "$current_town", slot_center_volunteer_troop_amount),
     (party_get_free_companions_capacity, ":free_capacity", "p_main_party"),
	 ...
	 (party_get_slot, ":volunteer_troop", "$current_town", slot_center_volunteer_troop_type),
	 (str_store_troop_name, s21, ":volunteer_troop"),
     (try_begin),
       # (eq, ":volunteer_amount", 0),
       # (str_store_string, s20, "@but none of them is"),
	 # (else_try),
	   (eq, ":volunteer_amount", 1),
	   (store_mul, reg6, ":volunteer_amount", 200),#200 denars per noble
	   (str_store_string, s20, "@only one {s21} is"),
     (else_try),
       (store_mul, reg6, ":volunteer_amount", 200),#200 denars per noble 
	   (str_store_string, s20, "@{reg5} {s21} are"),
	 (try_end),
	],
    [
      ("continue",[(eq, reg5, 0)],
      ...
      ("recruit_them",[(gt, reg5, 0)],
       "Recruit them ({reg6} denars).",[(call_script, "script_town_nobles_volunteers"),
                                        (jump_to_menu,"mnu_town"),
										(party_set_slot, "$current_town", slot_center_volunteer_troop_amount, -1),
                                        ]
	   ),
...
##diplomacy end+
  ...
  ("training_ground_training_result",mnf_disable_all_keys,
   ...
   "none",
   [
     (store_skill_level, ":trainer_skill", "skl_trainer_replacement", "trp_player"),
     ...
     (try_begin),
	   ...
	   (assign, ":xp_ratio_to_add_bonus", ":xp_ratio_to_add"),
       (try_begin),
         (gt, ":num_fit", 8),
...
##       (display_message, "@xp earn ratio: {reg0}/1000"),
       ...
       (try_begin),
         (eq, "$g_mt_mode", ctm_melee),
         (store_mul, ":special_xp_ratio_to_add", ":xp_ratio_to_add_bonus", 3),
  ...
  (
    ...
    [
	  ...
	  [
	    ...
	    (try_end),
		(try_for_range, ":unused", 0, ":number_of_veteran_bandits_will_be_spawned_at_each_period"),
		  (store_random_in_range, ":random_entry_point", 2, 11),	      
	      (set_visitor, ":random_entry_point", ":veteran_bandit_troop", 1),	      
	    (try_end),
...
##nested diplomacy end+
  ...
  (
	...
	##diplomacy end+
       ...
       [
      ("dplmc_disable_horse_speed",[(eq, "$g_dplmc_horse_speed", 0),],"Diplomacy horse speed enabled.",
       [
           (assign, "$g_dplmc_horse_speed", 1),
           (jump_to_menu, "mnu_dplmc_preferences"),
        ]),
      ("dplmc_enable_horse_speed",[(eq, "$g_dplmc_horse_speed", 1),],"Diplomacy horse speed disabled.",
       [
...
##diplomacy end+
      ...
      ("disable_wp_loss",[(eq, "$proficiency_loss", 1),],"Weapon proficiency loss : ON",
	    [(assign, "$proficiency_loss", 0),
	     (display_message, "@Proficiency loss OFF"),
	    ]
	  ),
	  ("enable_wp_loss",[(eq, "$proficiency_loss", 0),],"Weapon proficiency loss : OFF",
	    [(assign, "$proficiency_loss", 1),
	     (display_message, "@Proficiency loss ON"),
	    ]
	  ),
	  ("change_morale_type",
	    [
		  (try_begin),
		    (eq, "$g_morale_type", 1),
		    (str_store_string, s10, "@Change morale system (current is modded)"),
		  (else_try),
		    (str_store_string, s10, "@Change morale system (current is native)"),
		  (try_end),
		],"{s10}",
	    [
		  (try_begin),
		  ...
		  (else_try),
		    (assign, "$g_morale_type", 1),
		  (try_end),
		  (display_message, "@Changing the morale system in game may have odd consequences for a few in-game days."),
		  (jump_to_menu, "mnu_dplmc_preferences"),
	    ]
	  ),
	  
	  
	  ("banners_kingdom",[(eq, "$banner_type", 0),],"Switch to individual banners",
	    [(assign, "$banner_type", 1),
		
		 (call_script, "script_init_banners_slot", 0),
		 ...
		 (try_for_range, ":king", kings_begin, kings_end),
		   (troop_get_slot, ":banner_id", ":king", slot_troop_banner_scene_prop),
		   (store_sub, ":banner_icon_id", ":banner_id", banner_scene_props_begin),
		   (val_add, ":banner_icon_id", banner_map_icons_begin),
	       (troop_get_slot, ":party_no", ":king", slot_troop_leaded_party),
	       (try_begin),
	         (gt, ":party_no", 0),
	  ...
	  ("banners_alone",[(eq, "$banner_type", 1),],"Switch to kingdom based banners",
	    [(assign, "$banner_type", 0),
		 ...
		 (troop_set_slot, "trp_kingdom_1_lord", slot_troop_banner_scene_prop, "spr_banner_kingdom_a"),
		 ...
		 (troop_set_slot, "trp_kingdom_4_lord", slot_troop_banner_scene_prop, "spr_banner_kingdom_d"),
		 (troop_set_slot, "trp_kingdom_5_lord", slot_troop_banner_scene_prop, "spr_banner_kingdom_e"),
		 (troop_set_slot, "trp_kingdom_6_lord", slot_troop_banner_scene_prop, "spr_banner_kingdom_f"),
	  ...
	  ("party_order_troops_1", [(eq, "$order_parties", 0),], "Automaticaly order parties (current : OFF)",
	    [(assign, "$order_parties", 1),]
		),
	  ("party_order_troops_2", [(eq, "$order_parties", 1),], "Automaticaly order parties (current : Only lords)",
	    [(assign, "$order_parties", 2),]
		),
	  ("party_order_troops_3", [(eq, "$order_parties", 2),], "Automaticaly order parties (current : Lords and garrisons)",
...
##diplomacy end+  
  ...
  ("center_hire",0,
    "Set the maximum number of troops the center will be able to hire (remember it costs money)",
    "none",
    [
	],
    ...
    [
      ("100",[
       ...
       [         
		(party_set_slot, "$current_town", slot_center_max_troop, 100),
        (jump_to_menu, "mnu_town"),         
       ]),       
      ...
      ("150",[
       ...
       [     
        (party_set_slot, "$current_town", slot_center_max_troop, 150),	   
        (jump_to_menu, "mnu_town"),         
       ]),
	  ...
	  ("200",[
       ...
       [     
        (party_set_slot, "$current_town", slot_center_max_troop, 200),	   
        (jump_to_menu, "mnu_town"),         
       ]),
	  ...
	  ("250",[
       ...
       [     
        (party_set_slot, "$current_town", slot_center_max_troop, 250),	   
        (jump_to_menu, "mnu_town"),         
       ]),
	  ...
	  (party_slot_eq, "$current_town", slot_party_type, spt_town),
       ...
       [     
        (party_set_slot, "$current_town", slot_center_max_troop, 300),	   
        (jump_to_menu, "mnu_town"),         
       ]),
	  ...
	  (party_slot_eq, "$current_town", slot_party_type, spt_town),
       ...
       [     
        (party_set_slot, "$current_town", slot_center_max_troop, 350),	   
        (jump_to_menu, "mnu_town"),         
       ]),
	  ...
	  (party_slot_eq, "$current_town", slot_party_type, spt_town),
       ...
       [     
        (party_set_slot, "$current_town", slot_center_max_troop, 400),	   
        (jump_to_menu, "mnu_town"),         
       ]),
	  ...
	  (party_slot_eq, "$current_town", slot_party_type, spt_town),
       ...
       [     
        (party_set_slot, "$current_town", slot_center_max_troop, 450),	   
        (jump_to_menu, "mnu_town"),         
       ]),
	  ...
	  (party_slot_eq, "$current_town", slot_party_type, spt_town),
       ...
       [     
        (party_set_slot, "$current_town", slot_center_max_troop, 500),	   
        (jump_to_menu, "mnu_town"),         
       ]),
	  ...
	  ("nothing",[
       ...
       [     
        (party_set_slot, "$current_town", slot_center_max_troop, 0),	   
        (jump_to_menu, "mnu_town"),         
       ]),
    ]
  ),
  ...
  ("center_bank",0,
    "Here you can deposit and withdraw money. The money you deposit here will earn you denars each month.",
    "none",
    [
	],
    ...
    [
	  ("deposit",[
       ...
       [
        (jump_to_menu, "mnu_deposit"),         
       ]),
	   
	  ("withdraw",[
	  ],"Withdraw money",
       [
        (jump_to_menu, "mnu_withdraw"),         
       ]),
	  ...
	  ("intereset_store",[(party_get_slot, reg16, "$current_town", slot_center_bank_interest_stored),
	   ...
	   [
	    (val_add, reg16, 1),
		(val_mod, reg16, 2),
	    (party_set_slot, "$current_town", slot_center_bank_interest_stored, reg16),
	   ]),
  ...
  ("deposit",0,
    ...
    [
	  ("100",[(store_troop_gold, ":gold", "trp_player"),
	          (gt, ":gold", 100),
	  ],"Deposit 100 denars",
       [(troop_remove_gold, "trp_player", 100),
	    (party_get_slot, ":bank_money", "$current_town", slot_center_bank_money),
		(val_add, ":bank_money", 100),
		(party_set_slot, "$current_town", slot_center_bank_money, ":bank_money"),
       ]),
	   
	  ("500",[(store_troop_gold, ":gold", "trp_player"),
	          (gt, ":gold", 500),
	  ],"Deposit 500 denars",
       [(troop_remove_gold, "trp_player", 500),
	    (party_get_slot, ":bank_money", "$current_town", slot_center_bank_money),
		(val_add, ":bank_money", 500),
		(party_set_slot, "$current_town", slot_center_bank_money, ":bank_money"),
       ]),
	   
	  ("1000",[(store_troop_gold, ":gold", "trp_player"),
	           (gt, ":gold", 1000),
	  ],"Deposit 1000 denars",
       [(troop_remove_gold, "trp_player", 1000),
	    (party_get_slot, ":bank_money", "$current_town", slot_center_bank_money),
		(val_add, ":bank_money", 1000),
		(party_set_slot, "$current_town", slot_center_bank_money, ":bank_money"),
       ]),
	   
	  ("10000",[(store_troop_gold, ":gold", "trp_player"),
	           (gt, ":gold", 10000),
	  ],"Deposit 10000 denars",
       [(troop_remove_gold, "trp_player", 10000),
	    (party_get_slot, ":bank_money", "$current_town", slot_center_bank_money),
		(val_add, ":bank_money", 10000),
		(party_set_slot, "$current_town", slot_center_bank_money, ":bank_money"),
       ]),
    ...
    [
	  ...
	  ("500",[
       ...
       [(call_script, "script_troop_add_gold", "trp_player", 500),
	    (party_get_slot, ":bank_money", "$current_town", slot_center_bank_money),
		(val_sub, ":bank_money", 500),
		(party_set_slot, "$current_town", slot_center_bank_money, ":bank_money"),
       ]),
	   
	  ("1000",[
	  ],"Withdraw 1000 denars",
       [(call_script, "script_troop_add_gold", "trp_player", 1000),
	    (party_get_slot, ":bank_money", "$current_town", slot_center_bank_money),
		(val_sub, ":bank_money", 1000),
		(party_set_slot, "$current_town", slot_center_bank_money, ":bank_money"),
       ]),
	  
	  ("10000",[
	  ],"Withdraw 10000 denars",
       [(call_script, "script_troop_add_gold", "trp_player", 10000),
	    (party_get_slot, ":bank_money", "$current_town", slot_center_bank_money),
		(val_sub, ":bank_money", 10000),
		(party_set_slot, "$current_town", slot_center_bank_money, ":bank_money"),
       ]),
  ...
  ("kingdom_report_dead_lord_ally",mnf_scale_picture,
    "A messenger brings you the message announcing the death of {s31}, this tragic event raised amoung every one a painfull feeling. 'He will stay in our hearts forever' and 'He was a loyal vassal and a brave man' are the only words {s34} could pronounce before adding 'His death weaken us, but it should also strengthen our determination'.",
    "none",
    [
        (set_background_mesh, "mesh_pic_messenger"),
		(faction_get_slot, ":faction_leader", "$players_kingdom", slot_faction_leader),
		(str_store_troop_name, s34, ":faction_leader"),
      ...
      ("continue",[],"Continue...",
       [
           (try_for_range, ":lord_no", lords_begin, lords_end),
			 ...
			 (call_script, "script_change_player_relation_with_troop", ":lord_no", ":rand"),
		   (try_end),
		   (change_screen_return),
        ]),
     ]
  ...
  ("kingdom_report_dead_lord",mnf_scale_picture,
    "A messenger brings you the message announcing the death of {s31}, the messenger didn't said too much about it before leaving except that {s34} wanted you to know this.",
    "none",
    [
        (set_background_mesh, "mesh_pic_messenger"),
		(assign, ":faction", reg31),
  ...
  ("kill_lord",mnf_scale_picture,
    "You march slowly toward your target, an almost imperceptible smile seen on your lips. {s31}",
    "none",
    [
		(assign, ":troop_no", reg31),
		(try_begin),
		  (this_or_next|troop_slot_eq, ":troop_no", slot_troop_occupation, slto_kingdom_hero),
		  (is_between, ":troop_no", noble_troops_begin, noble_troops_end),
		  (str_store_string, s31, "str_kill_lord"),
		  (assign, reg35, 1),
		(else_try),
		  (party_get_num_prisoner_stacks, ":num_prisoner_stacks","p_main_party"),
		  (try_for_range_backwards, ":stack_no", 0, ":num_prisoner_stacks"),
		    (party_prisoner_stack_get_troop_id, ":cur_troop_id", "p_main_party", ":stack_no"),
			(eq, ":cur_troop_id", ":troop_no"),
		    (party_prisoner_stack_get_size, ":stack_size","p_main_party",":stack_no"),
			(try_begin),
			  (this_or_next|eq, ":stack_size", 1),
			  (eq, reg30, 1), # number of prisoner to kill !=1 means all of them
			  (str_store_string, s31, "str_kill_soldier"),
			  (assign, reg35, 1),
			(else_try),
			  (ge, ":stack_size", 4),
			  (str_store_string, s31, "str_kill_soldiers_lot"),
			  (assign, reg35, ":stack_size"),
			(else_try),
			  (str_store_string, s31, "str_kill_soldiers"),
			  (assign, reg35, ":stack_size"),
			(try_end),
	      (try_end),
		(try_end),
    ...
    [
	  ("kill",[
       ...
       [
	    (assign, reg30, 0),
		# (party_remove_prisoners, "p_main_party", reg31, reg35),
		# (call_script, "script_kill_prisoner_consequences", reg31),
	    (jump_to_menu, "mnu_kill_lord"),
        ]),
      ...
      ("continue",[],"Continue...",
       [
		   ...
		   (call_script, "script_kill_prisoner_consequences", reg31),
		   (try_begin),
		     (troop_slot_eq, reg31, slot_troop_occupation, slto_kingdom_hero),
			 (call_script, "script_kill_lord", reg31),
		   (try_end),
		   (change_screen_return),
        ]),
     ]
...
## bank begin
  ("bank_destroyed",0, # reg40 should be center, reg41 should be money
	...
	[(party_get_slot, reg42, reg40, slot_town_lord),
	 ...
	 (try_begin),
	   (lt, reg42, 1),
	   (faction_get_slot, reg42, reg44, slot_faction_leader),
	 (try_end),
	 (str_store_troop_name, s40, reg42),
	 (str_store_party_name, s41, reg40),
	 (troop_get_slot, ":lord_rel", reg42, slot_troop_player_relation),
	 (try_begin),
	   (lt, ":lord_rel", -50),
	   (store_mul, ":relation_bonus", ":lord_rel", 100),
	   (store_div, ":total_money", reg41, 15),
	 (else_try),
	   (lt, ":lord_rel", -5),
	   (store_mul, ":relation_bonus", ":lord_rel", 80),
	   (store_div, ":total_money", reg41, 10),
	 (else_try),
	   (gt, ":lord_rel", 50),
	   (store_mul, ":relation_bonus", ":lord_rel", 120),
	   (store_div, ":total_money", reg41, 2),
	 (else_try),
	   (gt, ":lord_rel", 5),
	   (store_mul, ":relation_bonus", ":lord_rel", 100),
	   (store_div, ":total_money", reg41, 5),
	 (else_try),
	   (store_mul, ":relation_bonus", ":lord_rel", 90),
	   (store_div, ":total_money", reg41, 8),
	 (try_end),
	 (val_add, ":total_money", ":relation_bonus"),
	 (val_min, ":total_money", 500),
	 (val_max, ":total_money", reg41),
	 (assign, reg43, ":total_money"),
	 ],
    [
	("accept",[],"Accept the money.",
       [
		...
		(party_add_template, ":result", "pt_kingdom_7_reinforcements_c"),
        (party_add_template, ":result", "pt_kingdom_7_reinforcements_c"),
		
		(party_set_ai_behavior, ":result", ai_bhvr_attack_party),
		(party_set_ai_object, ":result", "p_main_party"),
		(party_set_slot, ":result", slot_party_money_carried, reg43),
		(party_set_slot, ":result", slot_party_money_carried_state, 0),
	...
	("all",[(lt, reg43, reg41),],"Ask that everything has to be given to you ({reg41} denars).",
       [(call_script, "script_change_player_relation_with_troop", reg42, -3),
	    (call_script, "script_change_player_honor", -1),
		...
		(party_set_ai_behavior, ":result", ai_bhvr_attack_party),
		(party_set_ai_object, ":result", "p_main_party"),
		(party_set_slot, ":result", slot_party_money_carried, reg41),
		(party_set_slot, ":result", slot_party_money_carried_state, 0),
		(party_set_slot, ":result", slot_party_money_carried_lord, reg42),
		(change_screen_return),
	   ]),
     ]
  ),
