...
##diplomacy end
  ...
  ("camp_test", 0, 
   ...
   [
	  ...
	  ("disactivate_testing", [], "Disactivate testing mode",
	   [(assign, "$test", 0),
...
##########################
  ...
  "These nobles are the best warriors of all Calradia and the most expensive, {s20} willing to follow you",
  "none",
  [  (party_get_slot, ":volunteer_amount", "$current_town", slot_center_volunteer_troop_amount),
     (party_get_free_companions_capacity, ":free_capacity", "p_main_party"),
	 ...
	 (else_try),
	   ...
	   (str_store_string, s20, "@only one of them is"),
     (else_try),
       (store_mul, reg6, ":volunteer_amount", 200),#200 denars per noble 
	   (str_store_string, s20, "@{reg5} of them are"),
	 (try_end),
	],
    [
      ("continue",[(eq, reg5, 0)],
  ...
  ("bank_destroyed",0, # reg40 should be center, reg41 should be money
	...
	("accept",[],"Accept the money.",
       [(call_script, "script_change_player_relation_with_troop", reg42, 1),
		...
		(party_set_slot, reg0, slot_party_money_carried, reg43),
		(party_set_slot, reg0, slot_party_money_carried_state, 0),
		(change_screen_return),
	   ]),
	...
	("all",[(lt, reg43, reg41),],"State that you need everything that was in the bank ({reg4} denars).",
       [(call_script, "script_change_player_relation_with_troop", reg42, -3),
		...
		(party_set_ai_behavior, reg0, ai_bhvr_attack_party),
		(party_set_ai_object, reg0, "p_main_party"),
		(party_set_slot, reg0, slot_party_money_carried, reg41),
		(party_set_slot, reg0, slot_party_money_carried_state, 0),
		(party_set_slot, reg0, slot_party_money_carried_lord, reg42),
		(change_screen_return),
	   ]),
     ]
  ),
