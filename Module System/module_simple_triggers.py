   ...
   [
    (try_begin),
	  ...
	  (else_try),
	    ...
	    (troop_set_slot, "trp_player", slot_troop_banner_scene_prop, "spr_banner_do"),
	    (party_set_banner_icon, "p_main_party", "icon_banner_78"),
	  (else_try),
	    (eq, "$players_kingdom", "fac_kingdom_10"),
	    (troop_set_slot, "trp_player", slot_troop_banner_scene_prop, "spr_banner_cp"),
	    (party_set_banner_icon, "p_main_party", "icon_banner_58"),
	  (else_try),
	    (eq, "$players_kingdom", "fac_kingdom_11"),
	    (troop_set_slot, "trp_player", slot_troop_banner_scene_prop, "spr_banner_f"),
	    (party_set_banner_icon, "p_main_party", "icon_banner_06"),
	  (else_try),
	    (start_presentation, "prsnt_banner_selection"),
  #custom_banner_begin
  #    (start_presentation, "prsnt_custom_banner"),
      (try_end),
	(else_try), # player is a mercenary for one of the factions
	  (gt, "$players_kingdom", 0),
      (neq, "$players_kingdom", "fac_player_supporters_faction"),
	  ...
	  (try_begin),
		...
		(assign, ":banner_icon_id", "icon_map_flag_kingdom_f"),
      (else_try),
        (eq, "$players_kingdom", "fac_kingdom_2"),
        (assign, ":banner_id", "spr_banner_kingdom_b"),
		(assign, ":banner_icon_id", "icon_map_flag_kingdom_b"),
      (else_try),
        (eq, "$players_kingdom", "fac_kingdom_3"),
        (assign, ":banner_id", "spr_banner_kingdom_c"),
		(assign, ":banner_icon_id", "icon_map_flag_kingdom_c"),
      (else_try),
        (eq, "$players_kingdom", "fac_kingdom_4"),
        (assign, ":banner_id", "spr_banner_kingdom_a"),
		(assign, ":banner_icon_id", "icon_map_flag_kingdom_a"),
      (else_try),
        (eq, "$players_kingdom", "fac_kingdom_5"),
        (assign, ":banner_id", "spr_banner_kingdom_d"),
		(assign, ":banner_icon_id", "icon_map_flag_kingdom_d"),
      (else_try),
        (eq, "$players_kingdom", "fac_kingdom_6"),
        (assign, ":banner_id", "spr_banner_kingdom_e"),
		(assign, ":banner_icon_id", "icon_map_flag_kingdom_e"),
	  (else_try),
        (eq, "$players_kingdom", "fac_kingdom_9"),
        (assign, ":banner_id", "spr_banner_do"),
		(assign, ":banner_icon_id", "icon_banner_78"),
	  (else_try),
        (eq, "$players_kingdom", "fac_kingdom_10"),
        (assign, ":banner_id", "spr_banner_cp"),
		(assign, ":banner_icon_id", "icon_banner_58"),
      (else_try),
        (eq, "$players_kingdom", "fac_kingdom_11"),
        (assign, ":banner_id", "spr_banner_f"),
		(assign, ":banner_icon_id", "icon_banner_06"),
      (try_end),
	  (troop_set_slot, "trp_player", slot_troop_banner_scene_prop, ":banner_id"),
	  (party_set_banner_icon, "p_main_party", ":banner_icon_id"),
	(try_end),
  ...
  (24*7,
   [
       ...
       (try_for_range, ":center_no", walled_centers_begin, walled_centers_end),
         ...
         (store_mul, ":added_wealth", ":prosperity", 25),
         (val_add, ":added_wealth", 250),
         (try_begin),
  ...
  (24,
   [
     ...
     ##diplomacy end+
	   ...
	   (troop_slot_eq, ":troop_no", slot_troop_occupation, slto_kingdom_mercenary),
	   ...
	   # special elite mercenaries for kingdom lords - hireable in taverns too
	   (store_random_in_range, ":rand", 0, 12),
	   (try_begin),
		 (eq, ":rand", 0),
		 (troop_get_slot, ":troop_level", ":troop_no", slot_troop_lord_level),
		 (this_or_next|gt, ":troop_level", 3), # must be good lord
		 (is_between, ":troop_no", kings_begin, kings_end), # or king
		 
		 (party_get_num_companions, ":party_size", ":party_no"),
		 (call_script, "script_party_get_ideal_size", ":party_no"),
         # (store_div, ":minimum_size", reg0, 2), # must be over half the ideal size to recruit elite troops, these should come only when there is already a good number of troop in the party
		 (assign, ":maximum_size", reg0), # must not be over the ideal size
		 
		 # (gt, ":party_size", ":minimum_size"),
		 (lt, ":party_size", ":maximum_size"),
		 
		 (troop_get_slot, ":cur_wealth", ":troop_no", slot_troop_wealth),
		 (store_mul, ":hiring_budget", ":cur_wealth", 3),
		 (val_div, ":hiring_budget", 4),
		 (game_get_reduce_campaign_ai, ":reduce_campaign_ai"),
         (assign, ":reinforcement_cost", reinforcement_cost_moderate /4),
         (try_begin),
           (eq, ":reduce_campaign_ai", 0), #hard
           (assign, ":reinforcement_cost", reinforcement_cost_hard /4),
         (else_try),
           (eq, ":reduce_campaign_ai", 1), #moderate
           (assign, ":reinforcement_cost", reinforcement_cost_moderate /4),
         (else_try),
           (eq, ":reduce_campaign_ai", 2), #easy
           (assign, ":reinforcement_cost", reinforcement_cost_easy /4),
         (try_end),
		 (gt, ":hiring_budget", ":reinforcement_cost"),
		 (troop_get_slot, ":troop_to_add", ":troop_no", slot_troop_special_unit),
		 (gt, ":troop_to_add", 0),
     ...
     (try_for_range, ":center_no", walled_centers_begin, walled_centers_end),
	   ...
	   (try_begin),
		 ...
		 (assign, ":cur_wealth", 0),
         (party_slot_eq, ":center_no", slot_center_has_recruitment_camp, 1),
		 (party_get_num_companions, ":num_troop", ":center_no"),
		 (party_get_slot, ":max", ":center_no", slot_center_max_troop),
		 (gt, ":max", ":num_troop"),
		 (store_random_in_range, ":rand", 0, 2), # 50% chances to receive reinforcement when player has the recruitment camp
		 # it is slower to get troops with it than recruiting, but doesn't cost anything and get average quality troops (unless high relation with village)
		 (le, ":rand", 0), #needed because otherwise too much troops coming in the center
         (assign, ":cur_wealth", 1000),
       (try_end),
	   ...
	   (assign, ":continue", 0),
	   (try_begin), # centers with low garrison will get free reiforcement even if it belongs to noone
	     (party_get_num_companions, ":num_troops", ":center_no"),
		 (neg|party_slot_eq, ":center_no", slot_town_lord, "trp_player"), # the player could exploit it so we disable for him
		 (try_begin),
       ...
       (try_for_range, ":center_no", walled_centers_begin, walled_centers_end),
         ...
         (try_begin),
           ...
           (try_begin),
             ...
             (val_add, ":xp_gain", 50), # 10% more xp
           (else_try),
             (eq, ":reduce_campaign_ai", 2), #easy
             (assign, ":max_accepted_random_value", 25),
			 (val_sub, ":xp_gain", 50), # 10% less xp
           (try_end),
         (try_end),
         
		 (try_begin),
           (party_slot_eq, ":center_no", slot_center_has_trainer_1, 1),
		   (party_slot_eq, ":center_no", slot_center_has_trainer_2, 0),
		   (party_slot_eq, ":center_no", slot_center_has_trainer_3, 0),
		   (val_mul, ":xp_gain", 2), # doesn't bring much xp, but is run everytime
		   (party_upgrade_with_xp, ":center_no", ":xp_gain"),
		   (try_begin), # lords can build this upgrade too, so we hide the message if other lord is the owner
		     (party_slot_eq, ":center_no", slot_town_lord, "trp_player"),
		     (str_store_party_name, s2, ":center_no"),
		     (display_message, "@Training session at {s2} (lvl1)"),
		   (try_end),
		 (else_try),
		   (party_slot_eq, ":center_no", slot_center_has_trainer_2, 1),
		   (party_slot_eq, ":center_no", slot_center_has_trainer_3, 0),
		   (val_mul, ":xp_gain", 7), # much more xp than previously
		   (val_div, ":xp_gain", 2),
		   (party_upgrade_with_xp, ":center_no", ":xp_gain"),
		   (try_begin),
		     (party_slot_eq, ":center_no", slot_town_lord, "trp_player"),
		     (str_store_party_name, s2, ":center_no"),
		     (display_message, "@Training session at {s2} (lvl2)"),
		   (try_end),
		 (else_try),
		   ...
		   (try_begin),
		     (party_slot_eq, ":center_no", slot_town_lord, "trp_player"),
		     (str_store_party_name, s2, ":center_no"),
		     (display_message, "@Training session at {s2} (lvl3)"),
		   (try_end),
		 (else_try),
		   (store_random_in_range, ":rand", 0, 100),
		   (le, ":rand", ":max_accepted_random_value"),
...
##    ]),
  ...
  (48,
   [
       (call_script, "script_randomly_make_prisoner_heroes_escape_from_party", "p_main_party", 40),
       (try_for_range, ":center_no", walled_centers_begin, walled_centers_end),
##         (party_slot_eq, ":center_no", slot_town_lord, "trp_player"),
         (assign, ":chance", 20),
         (try_begin),
   ...
   [
	   ...
	   ##diplomacy end+
         ...
         (else_try),         
		   ...
		   (this_or_next|gt, ":num_towns", 0), # to prevent lords rom spawning after their kingdom is defeated
		   (gt, ":num_castles", 0),            # fix to the patch 1.142
         ...
         (else_try),
           ...
           (try_begin),
			 ...
			 # (neg|is_between, ":troop_no", "trp_kingdom_9_lord", kings_end),
             (troop_set_slot, ":troop_no", slot_troop_change_to_faction, "fac_commoners"),
           (else_try),
             (store_random_in_range, ":random_no", 0, 100),
             (lt, ":random_no", 10),
			 (neg|is_between, ":troop_no", "trp_knight_9_1", "trp_kingdom_1_pretender"),
 ...
 #Update how good a target player is for bandits
  ...
  (72,
   [
	 ...
	 (call_script, "script_update_smith"),
     ...
     (try_end),
##########################
## recruit nobles start ##
##########################
	 (try_for_range, ":town_no", towns_begin, towns_end),
	   (call_script, "script_update_nobles_troops_in_town", ":town_no"),
	 (try_end),
##########################
## recruit nobles end   ##
##########################
  ...
  (12,
   [(try_for_range, ":center_no", centers_begin, centers_end),
	  ...
	  (try_begin),
		...
		(call_script, "script_change_troop_renown", ":lord_no", 20),
      (try_end),
      # (try_begin),
        # (is_between, ":center_no", villages_begin, villages_end),
...
# School
  (14 * 24,
   [(try_for_range, ":cur_village", villages_begin, villages_end),
      ...
      (str_store_party_name, s7, ":cur_village"),
	  (display_message, "@Your relation with {s7} has improved due to the school you built"),
    (try_end),
    ]),
	
# Mill
  (30, # effects of ideal prosperity is reduced if prosperity>ideal
   [(try_for_range, ":cur_village", villages_begin, villages_end),
...
# Train peasants against bandits
  (1,
   [
     ...
     (call_script, "script_get_max_skill_of_player_party", "skl_trainer_replacement"),
   ...
   [           
        ...
        (else_try), #### Grievance
            ...
            (try_begin),
				...
				(troop_slot_ge, "$npc_with_grievance", slot_troop_morality_state, 1),  ##new 0.751
        ...
        (else_try),
            ...
            (eq, "$g_infinite_camping", 0),
            (eq, "$disable_npc_complaints", 0),
		    (troop_slot_ge, "$npc_with_personality_clash", slot_troop_personalityclash_object, 1),  ##new 0.751
        ...
        (else_try), #### Political issue
            ...
            (try_begin),
				...
				(troop_slot_ge, "$npc_with_political_grievance", slot_troop_kingsupport_objection_state, 1),  ##new 0.751
...
#           (try_end),
       ...
       (try_begin),
		 ...
		 (val_add, ":main_party_morale", ":num_deserters_total"),
		 (party_set_morale, "p_main_party", ":main_party_morale"),
 ...
 # reserved for future use. For backward compatibility, we need to use these triggers instead of creating new ones.
  ...
  (24*7, # proficiency loss
   [(eq, "$proficiency_loss", 1),
    (try_for_range, ":proficiency", 0, 6),
      (store_proficiency_level, ":wp", "trp_player", ":proficiency"),
	  (ge, ":wp", 75),
	  (store_div, ":wp_div_25", ":wp", 25),
	  (store_random_in_range, ":rand_wp_max", 0, ":wp_div_25"),
	  (store_random_in_range, reg5, 0, ":rand_wp_max"),
	  (val_mul, reg5, -1),
	  (troop_raise_proficiency_linear, "trp_player", ":proficiency", reg5),
	  (try_begin),
	    (eq, ":proficiency", 0),
		(str_store_string, s10, "@mastery in one handed weapons"),
	  (else_try),
	    (eq, ":proficiency", 1),
		(str_store_string, s10, "@mastery in two handed weapons"),
	  (else_try),
	    (eq, ":proficiency", 2),
		(str_store_string, s10, "@mastery in polearms"),
	  (else_try),
	    (eq, ":proficiency", 3),
		(str_store_string, s10, "@mastery in archery"),
	  (else_try),
	    (eq, ":proficiency", 4),
		(str_store_string, s10, "@mastery in crossbows"),
	  (else_try),
	    (eq, ":proficiency", 5),
		(str_store_string, s10, "@mastery in throwing weapons"),
	  (try_end),
	  (lt, reg5, 0),
	  (display_message, "str_you_lost_reg5_s10"),
	(try_end),
   ]),
  ...
  (24*7,
   [(try_for_range, ":center_no", centers_begin, centers_end),
      (party_get_slot, ":lord_no", ":center_no", slot_town_lord),
	  (is_between, ":lord_no", active_npcs_begin, active_npcs_end),
	  (troop_get_slot, ":lrep", ":lord_no", slot_lord_reputation_type),
	  (assign, ":chance", 200),
	  (try_begin),
		...
		(val_div, ":chance", 2),
      (else_try),
	    (eq, ":lrep", lrep_selfrighteous),
		(val_mul, ":chance", 5),
		(val_div, ":chance", 2),
	  (else_try),
	    (eq, ":lrep", lrep_debauched),
		(val_mul, ":chance", 2),
	  (else_try),
		...
		(val_div, ":chance", 3),
	  (else_try),
	    (eq, ":lrep", lrep_upstanding),
		(val_mul, ":chance", 4),
		(val_div, ":chance", 5),
	  (try_end),
	  (store_random_in_range, ":rand", 0, ":chance"),
	  (eq, ":rand", 0),
	  (try_begin),
	    (party_slot_eq, ":center_no", slot_party_type, spt_village),
		(store_random_in_range, ":improvement", village_improvements_begin, village_improvements_end),
	  (else_try),
	    # (party_slot_eq, ":center_no", slot_party_type, spt_castle),
		...
		(try_begin),
		  ...
		  (party_slot_eq, ":center_no", slot_center_has_trainer_1, 0),
		  (assign, ":improvement", -1),
		(else_try),
		  (eq, ":improvement", slot_center_has_trainer_3),
		  (party_slot_eq, ":center_no", slot_center_has_trainer_2, 0),
		  (assign, ":improvement", -1),
		(else_try),
		  (party_slot_eq, ":center_no", slot_party_type, spt_castle),
		  (this_or_next|eq, ":improvement", slot_center_has_trainer_3),
	  ...
	  (gt, ":improvement", 0),
	  ##
	  (neq, ":improvement", slot_center_has_barrack),
	  ##
	  (party_slot_eq, ":center_no", ":improvement", 0),
	  (party_set_slot, ":center_no", slot_center_current_improvement, ":improvement"),
      (store_current_hours, ":cur_hours"),
      (store_mul, ":hours_takes", 7, 24),
	  ...
	  (try_begin),
		...
		# (assign, reg10, ":improvement"),
		(call_script, "script_get_improvement_details", ":improvement"),
		(display_message, "@Building improvement in {s10} ({s0})"),
	  (try_end),
	(try_end),
	]),
   
   
  (2, ## decrease hours_to_finish each 2 hours for the smith (repairing weapons/armors)
   [(try_for_range, ":cur_center", towns_begin, towns_end),
      (party_slot_ge, ":cur_center", slot_center_smith_item, 1),
	  (party_get_slot, ":time_to_finish", ":cur_center", slot_center_hours_to_finish),
	  (val_add, ":time_to_finish", -2),
	  (val_max, ":time_to_finish", 0),
	  (party_set_slot, ":cur_center", slot_center_hours_to_finish, ":time_to_finish"),
	(try_end),
	]),
  ...
  (6, # patrol - scout behavior
   [(try_for_parties, ":party_no"),
	  ...
	  (neg|party_slot_ge, ":party_no", dplmc_slot_party_mission_diplomacy, 0), ## only change behavior of automatic patrols
	  ...
	  (try_begin),
		...
		(assign, "$g_move_heroes", 1),
		(call_script, "script_party_prisoners_add_party_prisoners", ":center", ":party_no"),
		(assign, "$g_move_heroes", 1),
		(call_script, "script_party_remove_all_prisoners", ":party_no"),
		  
		(try_begin),
		  (this_or_next|le, ":num_companions", 5), # Very small patrols or scout should disapear
		  (party_slot_eq, ":party_no", slot_party_type, spt_patrol),
		  
		  (lt, ":num_companions", 20), # Small patrols should disapear
		  
		  # In case the center they are in has a small garrison, we will add these troops to it
		  (party_get_num_companions, ":num_center_companions", ":center"),
		  (try_begin),
		    (lt, ":num_center_companions", 250),
			(call_script, "script_party_add_party_companions", ":center", ":party_no"),
		  (try_end),
	  ...
	  (try_begin),
	    ...
	    (try_begin),
	      (gt, ":marshall", 0),
	      (troop_get_slot, ":marshall_party", ":marshall", slot_troop_leaded_party),
	    (try_end),
	    
	    (assign, ":best_dist", 1000),
	    (assign, ":best_center_dist", 1000),
	    (try_for_parties, ":potential_target"),
	      (neq, ":potential_target", ":party_no"),
		  (store_faction_of_party, ":faction_no", ":potential_target"),
		  (assign, ":continue", 0),
		  (try_begin), # the player is in a different kingdom than its followers
		    (eq, ":party_faction", "fac_player_supporters_faction"),
		    (eq, ":faction_no", "fac_player_faction"),
		    (assign, ":continue", 1),
		  (try_end),
		  (this_or_next|eq, ":party_faction", ":faction_no"),
		  (eq, ":continue", 1),
		  (assign, ":center_dist", 1000),
		  
		  (store_distance_to_party_from_party, ":dist", ":party_no", ":potential_target"),
		  (assign, ":object", ":potential_target"),
		  (try_begin),
		    (party_slot_eq, ":potential_target", slot_party_type, spt_castle),
		    (assign, ":center_dist", ":dist"),
		    (val_add, ":dist", 2),
		    (assign, ":action", ai_bhvr_patrol_party),
		  (else_try),
		    (party_slot_eq, ":potential_target", slot_party_type, spt_town),
		    (assign, ":center_dist", ":dist"),
		    (assign, ":action", ai_bhvr_patrol_party),
		  (else_try),
		    (party_slot_eq, ":potential_target", slot_party_type, spt_village),
		    (val_add, ":dist", 5),
		    (assign, ":action", ai_bhvr_patrol_party),
		  (else_try),
		    (party_slot_eq, ":potential_target", slot_party_type, spt_kingdom_caravan),
		    (val_add, ":dist", 5),
		    (assign, ":action", ai_bhvr_escort_party),
		  (else_try),
		    (this_or_next|party_slot_eq, ":potential_target", slot_party_type, spt_kingdom_hero_party),
		    (eq, ":potential_target", "p_main_party"), ## allow patrols to escort player party without being asked
		    (val_add, ":dist", 10),
		    (assign, ":action", ai_bhvr_escort_party),
		    (try_begin),
		      (eq, ":leader_party", ":potential_target"),
		  	  (val_sub, ":dist", 3), # more likely to follow king
		    (else_try),
		      (eq, ":marshall_party", ":potential_target"),
		  	(val_sub, ":dist", 4), # more likely to follow marshall
		    (try_end),
		    (get_party_ai_behavior, ":cur_behavior", ":potential_target"),
		    (try_begin),
              (eq, ":cur_behavior", ai_bhvr_in_town),
		  	  (val_add, ":dist", 10), # lords in centers don't need patrols
		    (try_end),
		  (else_try),
		    (party_slot_eq, ":potential_target", slot_party_type, spt_village_farmer),
		    (val_add, ":dist", 9),
		    (assign, ":action", ai_bhvr_escort_party),
		  (else_try),
		    (party_slot_eq, ":potential_target", slot_party_type, spt_patrol),
		    (party_slot_eq, ":party_no", slot_party_type, spt_scout),
		    (val_add, ":dist", 18),
		    (assign, ":action", ai_bhvr_escort_party),
		  (else_try),
		    (val_add, ":dist", 1000), # should never use other party types because of this
		  (try_end),
		  
		  (try_begin),
		    (lt, ":center_dist", ":best_center_dist"),
		    (assign, ":best_center_dist", ":center_dist"),
		    (assign, ":nearest_center", ":object"),
		  (try_end),
		  
		  (lt, ":dist", ":best_dist"),
		  (assign, ":best_dist", ":dist"),
		  (assign, ":best_action", ":action"),
          (assign, ":best_object", ":object"),
	    (try_end),
	    
	    (store_random_in_range, ":rand", 0, 50),
	    (try_begin), # to make patrols and scout deposit their prisoners if they have too many
	      (this_or_next|gt, ":num_prisoners", ":rand"),
		  (lt, ":num_companions", ":num_prisoners"),
	      (party_set_ai_behavior, ":party_no", ai_bhvr_travel_to_party),
          (party_set_ai_object, ":party_no", ":nearest_center"),
	    (else_try),
	      (lt, ":best_dist", 1000),
	      (party_set_ai_behavior, ":party_no", ":best_action"),
          (party_set_ai_object, ":party_no", ":best_object"),
	    (else_try),
	      (party_set_ai_behavior, ":party_no", ai_bhvr_patrol_location),
          (party_set_ai_object, ":party_no", 0),
	    (try_end),
	  (try_end),
	(try_end),
	  ]),
	  
  (3, # decide mercenary ai
   [
    (try_for_range, ":merc_no", kingdom_mercenaries_begin, kingdom_mercenaries_end),
	  (neg|troop_slot_ge, ":merc_no", slot_troop_prisoner_of_party, 0),
	  (troop_get_slot, ":party_no", ":merc_no", slot_troop_leaded_party),
	  
	  (store_troop_faction, ":faction_merc", ":merc_no"),
	  (eq, ":faction_merc", "fac_neutral_4"),
	  (try_begin),
	    (troop_slot_eq, ":merc_no", slot_troop_occupation, slto_kingdom_mercenary),
		(lt, ":party_no", 0),
		(assign, ":center", 0),
		(assign, ":tries_end", 5),
		(try_for_range, ":unused", 0, ":tries_end"),
		  (store_random_in_range, ":potential_center", towns_begin, towns_end),
		  (neg|party_slot_eq, ":potential_center", slot_village_state, svs_under_siege), # only small chances are that a merc will go into a besieged center
		  (assign, ":center", ":potential_center"),										 # can still hapen if it gets besieged after he's already inside
		  (assign, ":tries_end", 0),
		(try_end),
		(neq, ":center", 0),
		(call_script, "script_create_kingdom_hero_party", ":merc_no", ":center"),
		# Spawn troop
	  (try_end),
	  (neg|troop_slot_ge, ":merc_no", slot_troop_mercenary_time, 1),
	  (gt, ":party_no", 0),
	  (get_party_ai_behavior, ":behavior", ":party_no"),
	  # (get_party_ai_object, ":object", ":party_no"),
	  (try_begin),
	    (troop_slot_eq, ":merc_no", slot_troop_occupation, slto_kingdom_mercenary),
		(troop_slot_eq, ":merc_no", slot_troop_mercenary_time, -1),
		(troop_slot_eq, ":merc_no", slot_troop_next_mercenary_faction, "fac_player_supporters_faction"),
		(try_begin),
	  ...
	  (else_try),
		...
		(try_begin),
		  ...
		  (try_end),
		  (try_begin),
			(call_script, "script_cf_mercenary_join_centers_faction", ":merc_no", ":town"),
		...
		(try_end),
	  (else_try),
	    (eq, ":behavior", ai_bhvr_patrol_location),
  ...
  (24,
   [(try_for_parties, ":party_no"),
      (party_is_active, ":party_no"),
	  (party_get_battle_opponent, ":opponent", ":party_no"),
	  (lt, ":opponent", 0), # party is not fighting someone else (if not will cause ai armies to be cleared of its wounded members
	  ...
	  (try_for_range, ":unused", 0, ":num_tries"),
	    (call_script, "script_order_party_troops", ":party_no"),
		(eq, reg0, 1),
		(assign, ":num_tries", 0), # no more troops moving
	  (try_end),
    (try_end),]),
	
	
  (24, #replacement for trainer's skill, take food quality as a modifier for experience gained
   [
    (assign, ":total_bonus", 0),
    (try_for_range, ":cur_edible", food_begin, food_end), # first get player's food bonus
      (call_script, "script_cf_player_has_item_without_modifier", ":cur_edible", imod_rotten),
      (item_get_slot, ":food_bonus", ":cur_edible", slot_item_food_bonus),
	  (val_add, ":total_bonus", ":food_bonus"),
	(try_end),
	
	(try_begin),
	  (eq, ":total_bonus", 0),
	  (display_message, "@The troops refused to train due to the lack of food", 0xeebbbb),
	(else_try),
      (assign, ":total_xp", 0),
	  ...
	  (try_for_range, ":trainer_stack", 0, ":num_stack"),
	    ...
	    (try_for_range, ":cur_stack", 0, ":num_stack"),
	      ...
	      (gt, ":trainer_level", ":troop_level"), # only train troops that have lower level than the trainer
	      (party_stack_get_size, ":stack_size", "p_main_party", ":cur_stack"),
	      (store_mul, ":experience_gained", ":experience", ":stack_size"),
	      (party_add_xp_to_stack, "p_main_party", ":cur_stack", ":experience_gained"),
	  	(val_add, ":total_xp", ":experience_gained"),
	    (try_end),
	  (try_end),
	  (try_begin),
	    (gt, ":total_xp", 0),
	    (assign, reg21, ":total_xp"),
	    (display_message, "@Day's training is complete..."),
	    (display_message, "@Total experience gained through training: {reg21}"),
	    (try_begin), # training less effective when not enough food
	      (lt, ":total_bonus", 15),
  ...
  (7*24, # correct banners on centers
   [(try_for_range, ":center_no", walled_centers_begin, walled_centers_end),
  ...
  (24,
   [(val_sub, "$morale_prisoner", 1),
    (val_max, "$morale_prisoner", -1),
	(store_random_in_range, "$daily_random", 0, 10000), ## store a random number that can be used in various situations -- right now only used for random prices for ramun the slave trader
	]),
  ...
  (6,
   [(eq, "$test", 1),
    (try_for_range, ":lord_no", active_npcs_begin, active_npcs_end),
      (troop_slot_eq, ":lord_no", slot_troop_money_tracked, 1),
	  (troop_get_slot, reg20, ":lord_no", slot_troop_wealth),
	  (str_store_troop_name, s15, ":lord_no"),
	  (display_message, "@Money of {s15} : {reg20} denars"),
	(try_end),]),
   
  (24, # every day check the end time for a mercenary to end its contract, remove from faction if so
   [(try_for_range, ":merc_no", kingdom_mercenaries_begin, kingdom_mercenaries_end),
      (troop_slot_eq, ":merc_no", slot_troop_occupation, slto_kingdom_mercenary),
	  (troop_get_slot, ":mercenary_time", ":merc_no", slot_troop_mercenary_time),
	  (try_begin),
	    (gt, ":mercenary_time", 0),
	    (val_sub, ":mercenary_time", 1), # decrease mercenary time
	    (troop_set_slot, ":merc_no", slot_troop_mercenary_time, ":mercenary_time"),
	  (try_end),
	  (store_troop_faction, ":faction_no", ":merc_no"),
	  (neq, ":faction_no", "fac_neutral_4"), # no need to check for factionless mercenaries
	  ...
	  (try_begin),
		...
		(try_begin),
		  (assign, ":continue", 0),
		  (try_begin), # currently in player's faction
		    (eq, ":faction_no", "fac_player_supporters_faction"),
		    (eq, ":leader", "trp_player"),
			(troop_set_slot, ":merc_no", slot_troop_next_mercenary_faction, "fac_player_supporters_faction"),
			(assign, ":continue", 1),
		  (else_try), # or switching to it
		    (troop_slot_eq, ":merc_no", slot_troop_next_mercenary_faction, "fac_player_supporters_faction"),
			(assign, ":continue", 1),
		  
		  (try_end),
		  (eq, ":continue", 1),
		  (troop_set_slot, ":merc_no", slot_troop_mercenary_time, -1),
		  # (troop_set_faction, ":merc_no", "fac_neutral_4"), # we don't want him to get attacked before he gets to the player or to attack him when he sees him !
		  (try_begin), # if he's in player's faction, or about to change for it, make him talk to the the player
		    (gt, ":party_no", 0),
			# (party_set_faction, ":party_no", "fac_neutral_4"),
		    (party_set_ai_behavior, ":party_no", ai_bhvr_attack_party),
            (party_set_ai_object, ":party_no", "p_main_party"),
		  (try_end),
		  
		(else_try), # leaving current faction
		  (neq|call_script, "script_cf_mercenary_keep_current_contract", ":merc_no"),
		  (troop_set_faction, ":merc_no", "fac_neutral_4"),
		  (troop_set_slot, ":merc_no", slot_troop_next_mercenary_faction, 0),
		  (try_begin),
		    (gt, ":party_no", 0),
		    (party_set_faction, ":party_no", "fac_neutral_4"),
		  (try_end),
		  (try_begin),
	        (eq, "$test", 1),
	        (display_message, "@Mercenary contract expired"),
	      (try_end),
		  
		(else_try), # keeping current contract if all else fail
		  (troop_set_slot, ":merc_no", slot_troop_mercenary_time, 30), # just renew the contract for another month
		(try_end),
	  (try_end),
    (try_end),
	]
   ...
   #daedalus end
      ...
      (try_begin),
         ...
         (try_for_range, ":village", villages_begin, villages_end),
            ...
            #daedalus begin
			   (store_faction_of_party, ":village_faction", ":village"),  ## use current faction instead
      ...
      #daedalus begin
			...
			(store_faction_of_party, ":target_faction", ":target"),  ## use current faction instead
            (assign,":stop",1),
            (try_begin),
            (eq,":recruit_faction",-1),
            (assign,":stop",0),
...
(12,
  ...
  (12,
   [  (try_begin),
		...
		(val_add, ":cur_cloud_amount", ":rand"),
		(val_min, ":cur_cloud_amount", 100),
	    (set_global_cloud_amount, ":cur_cloud_amount"),
		(try_begin),
		  (lt, ":rand", 5),
		  (assign, "$cloud_change", 0),
		(try_end),
	  (else_try),
	    (eq, "$cloud_change", 2),
	    (store_random_in_range, ":rand", 0, 20),
		(get_global_cloud_amount, ":cur_cloud_amount"),
		(val_sub, ":cur_cloud_amount", ":rand"),
		...
		(try_end),
	  (else_try),
	    (store_random_in_range, ":rand", -4, 5),
		(get_global_cloud_amount, ":cur_cloud_amount"),
		(val_add, ":cur_cloud_amount", ":rand"),
		(val_min, ":cur_cloud_amount", 100),
		(val_max, ":cur_cloud_amount", 0),
		...
		(try_begin),
		  (eq, ":rand", -4),
		  (assign, "$cloud_change", 2),
		(else_try),
		  (eq, ":rand", 4),
		  (assign, "$cloud_change", 1),
		(try_end),
	  (try_end),
	  (try_begin),
	    (eq, "$fog_change", 1),
  ...
  (24, ## No Banner change, correcting old faction banner
   [(eq, "$banner_type", 0), # only run this script if we have kingdom banners
    (try_for_range, ":troop_no", active_npcs_begin, pretenders_begin),
	  (this_or_next|troop_slot_eq, ":troop_no", slot_troop_occupation, slto_kingdom_hero),
	  (troop_slot_eq, ":troop_no", slot_troop_occupation, slto_kingdom_mercenary),
	  
	  (call_script, "script_assign_banner", ":troop_no"),
	(try_end),
	]),
	
  (24, ## always at least 5 parties for a kingdom (except king) = (10*num_town + 4*num_castle)/3
   [(try_for_range, ":faction_no", kingdoms_begin, kingdoms_end),
      # (neq, ":faction_no", "fac_player_supporters_faction"),
      ...
      (try_for_range, ":lord_no", heroes_begin, heroes_end), # Fix for promoted companions
	    ...
	    (try_begin),
		  ...
		  (val_add, ":num_lord", 1),
		(else_try),
		  (troop_slot_eq, ":lord_no", slot_troop_occupation, slto_kingdom_mercenary),
		  (val_add, ":num_merc", 1),
		(try_end),
	  ...
	  (try_begin),
	    (eq, "$test", 1),
	    (store_add, reg6, ":num_lord", ":num_merc"),
		(assign, reg7, ":num_lord"),
		(assign, reg8, ":num_merc"),
	    (str_store_faction_name, s10, ":faction_no"),
	    (display_message, "@{s10}: {reg6} parties ({reg7} lords, {reg8} mercenaries)"),
	  (try_end),
	  (faction_get_slot, ":num_center", ":faction_no", slot_faction_num_center),
	  # (faction_slot_ge, ":faction_no", slot_faction_num_center, ":num_lord"),
	  # (store_add, ":faction_rebel_no", ":faction_no", -9), ## this is for a future feature
	  # (lt, ":num_lord", 25),
	  (try_begin),
	    (ge, ":num_center", ":num_lord"),
	    (store_random_in_range, ":new_lord", lords_begin, lords_end),
	    (troop_slot_eq, ":new_lord", slot_troop_original_faction, ":faction_no"),
	    # (this_or_next|troop_slot_eq, ":new_lord", slot_troop_original_faction, ":faction_no"),
	    # (troop_slot_eq, ":new_lord", slot_troop_original_faction, ":faction_rebel_no"),
	    (troop_slot_eq, ":new_lord", slot_troop_occupation, slto_inactive),
	    
	    (call_script, "script_ready_new_lord", ":new_lord", ":faction_no"),
	  (try_end),
	  ...
	  (try_begin),
	    (eq, ":faction", "fac_neutral_4"), # neutral mercenary starting faction
		(troop_slot_eq, ":mercenary", slot_troop_occupation, 0),
	    (call_script, "script_change_troop_faction", ":mercenary", ":faction_no"),
	    (troop_set_slot, ":mercenary", slot_troop_occupation, slto_kingdom_mercenary),
	    (troop_set_slot, ":mercenary", slot_troop_mercenary_time, 30), # engage a 30 days mercenary contract
	    (store_random_in_range, ":intermediate_random", 1, 8),
	    (store_random_in_range, ":random_level", 1, ":intermediate_random"), # do not let level 0 mercenaries spawn
		(store_random_in_range, ":reputation_type", 13, 17),
		(try_begin),
		...
		(troop_set_slot, ":mercenary", slot_troop_mercenary_payment_for_player, ":random_level"),
	    (troop_set_slot, ":mercenary", slot_troop_spawned_before, 0), # mercenaries spawn with their own troops
		(call_script, "script_set_attrib_points", ":mercenary"),  ## change stats before
	    (call_script, "script_change_equipement", ":mercenary"),
		(call_script, "script_change_name_of_adventurer", ":mercenary"),
		
		(try_begin),
	      (eq, ":faction_no", "fac_kingdom_1"),
	      (assign, ":banner", "spr_banner_kingdom_f"),
        (else_try),
          (eq, ":faction_no", "fac_kingdom_2"),
          (assign, ":banner", "spr_banner_kingdom_b"),
        (else_try),
          (eq, ":faction_no", "fac_kingdom_3"),
          (assign, ":banner", "spr_banner_kingdom_c"),
        (else_try),
          (eq, ":faction_no", "fac_kingdom_4"),
          (assign, ":banner", "spr_banner_kingdom_a"),
        (else_try),
          (eq, ":faction_no", "fac_kingdom_5"),
          (assign, ":banner", "spr_banner_kingdom_d"),
        (else_try),
          (eq, ":faction_no", "fac_kingdom_6"),
          (assign, ":banner", "spr_banner_kingdom_e"),
	    (else_try),
          (eq, ":faction_no", "fac_kingdom_9"),
          (assign, ":banner", "spr_banner_do"),
	    (else_try),
          (eq, ":faction_no", "fac_kingdom_10"),
          (assign, ":banner", "spr_banner_cp"),
        (else_try),
          (eq, ":faction_no", "fac_kingdom_11"),
          (assign, ":banner", "spr_banner_f"),
        (try_end),
		...
		(try_begin),
		  (eq, "$test", 1),
		  (display_message, "@Mercenay recruited for {s10}"),
		(try_end),
	  # (else_try),
	  (try_end),
	(try_end),
  ...
  (24, ## change names corresponding to the occupation
   [(try_for_range, ":lord_no", heroes_begin, heroes_end),  # companions can have changing names too
      ...
      (try_for_range, ":cur_center", centers_begin, centers_end),
		...
		(try_end),
	  (try_end),
	  (troop_set_slot, ":lord_no", slot_troop_lord_level, ":level"),
	  (val_min, ":level", 5),
	  (call_script, "script_get_name_for_heroes", ":lord_no", ":level"),
	(try_end),
	]),
   ...
   (24*7,  # trigger to change equipement  ## TRIGGER 115
    [
	 ...
	 (try_for_range, ":lord_no", lords_begin, heroes_end),
	   ...
	   (try_end),
	   (neq, ":new", 0),
	   (store_add, ":equipement_level_new", ":equipement_level", ":new"),
	   (is_between, ":equipement_level_new", 0, 7), ## other ways should never happen
	   ...
	   (try_begin),
	     ...
	     (assign, reg11, ":equipement_level_new"),
		 (assign, reg10, ":equipement_level"),
	     (display_message, "@{s11} now level {reg11} (from {reg10})"),
	   (try_end),
	 (try_end),
	]),
	
   (24*7, ##add lords to player faction  ## TRIGGER 116
    [(assign, ":not", 1),
	 (try_for_range, ":center",centers_begin, centers_end),
	   (store_faction_of_party, ":center_faction", ":center"),
	   (eq, ":center_faction", "fac_player_supporters_faction"),
	 ...
	 (val_div, ":renown", 350), # renown not over 2000 ? - so 6% bonus chance max
	 (store_div, ":chances", "$player_right_to_rule", 3),
	 (val_add, ":chances", ":renown"), # 5 lords, chances decrease everytime one get into player's faction
	 (gt, ":chances", ":rand"),  # 33 + 6 = 39% max -> 31% -> 23% -> 16% -> 8% for the last lord
	 (store_random_in_range, ":troop_no", "trp_knight_7_1", "trp_adventurer_01"), # 5 free lords available
	 (store_troop_faction, ":fac", ":troop_no"),
	 (eq, ":fac", "fac_neutral"),
	 (call_script, "script_change_troop_faction", ":troop_no", "fac_player_supporters_faction"),
	 (store_random_in_range, ":rep", 0, 8),
	 (try_begin),
	   (eq, ":rep", 0),
	   (assign, ":rep", 1),
	 (try_end),
	 (troop_set_slot, ":troop_no", slot_lord_reputation_type, ":rep"),
	 (troop_set_slot, ":troop_no", slot_troop_equipement_level, 0), # start at lowest level, player has to give them fief for them to become powerfull
	 (troop_set_slot, ":troop_no", slot_troop_spawned_before, 1), # they spawn with no troops
	 (call_script, "script_set_attrib_points", ":troop_no"),
	 (call_script, "script_change_equipement", ":troop_no"),
	...
	(1, ## check if adventurer is in a city to buy equipments
    [
	 # (eq, 0, 1), # disabled for now
	 # (try_for_range, ":troop_no", "trp_adventurer_01", "trp_town_1_seneschal"),
	   # (neg|troop_slot_ge, ":troop_no", slot_troop_prisoner_of_party, 1),
	   # (troop_get_slot, ":party_no", ":troop_no", slot_troop_leaded_party),
	   # (party_get_attached_to, ":cur_attached_party", ":party_no"),
       # (is_between, ":cur_attached_party", towns_begin, towns_end),
	   
	   # (troop_get_slot, ":next_action", ":troop_no", slot_troop_next_action_preference),
	   # (val_and, ":next_action", 0x0f),
	   # (gt, ":next_action", 0x04),
	   
	   # (store_sub, ":add_city", ":cur_attached_party", "p_town_1"),
	   # (store_add, ":current_weapon_merchant", ":add_city", "trp_town_1_weaponsmith"),
	   # (call_script, "script_cf_check_best_equipment_for_adventurer", ":current_weapon_merchant", ":troop_no"),
	   # (assign, ":best_weapon", reg4),
	   # (assign, ":weapon_to_remove", reg5),
	   
	   # (store_add, ":current_armor_merchant", ":add_city", "trp_town_1_armorer"),
	   # (call_script, "script_cf_check_best_equipment_for_adventurer", ":current_armor_merchant", ":troop_no"),
	   # (assign, ":best_armor", reg4),
	   # (assign, ":armor_to_remove", reg5),
	   
	   # (store_add, ":current_horse_merchant", ":add_city", "trp_town_1_horse_merchant"),
	   # (call_script, "script_cf_check_best_equipment_for_adventurer", ":current_horse_merchant", ":troop_no"),
	   # (assign, ":best_horse", reg4),
	   # (assign, ":horse_to_remove", reg5),
	   
	   # (troop_get_slot, ":slot_a", ":troop_no", slot_troop_culture_type),
	   # (val_mul, ":slot_a", 20),
	   # (troop_get_slot, ":slot_b", ":troop_no", slot_troop_adventurer_type),
	   # (store_add, ":total_slot", ":slot_a", ":slot_b"),
		
	   # (item_get_slot, ":weapon_like", ":best_weapon", ":total_slot"),
	   # (item_get_slot, ":armor_like", ":best_armor", ":total_slot"),
	   # (item_get_slot, ":horse_like", ":best_horse", ":total_slot"),
	   # (try_begin),
	     # (gt, ":armor_like", ":weapon_like"),
		 # (gt, ":armor_like", ":horse_like"),
		 # (assign, ":best_weapon", ":best_armor"),
		 # (assign, ":weapon_to_remove", ":armor_to_remove"),
	   # (else_try),
	     # (gt, ":horse_like", ":weapon_like"),
		 # (gt, ":horse_like", ":armor_like"),
		 # (assign, ":best_weapon", ":best_horse"),
		 # (assign, ":weapon_to_remove", ":horse_to_remove"),
	   # (try_end),
	   
	   # (gt, ":best_weapon", 0),
	   
	   # (troop_get_slot, ":money_carried", ":troop_no", slot_troop_gold_carried),
	   # (item_get_slot, ":base_price", ":best_weapon", slot_item_base_price),
	   # (item_get_slot, ":old_item_price", ":weapon_to_remove", slot_item_base_price),
	   
	   # (val_sub, ":money_carried", ":base_price"),
	   
	   # (party_get_skill_level, ":trade_skill", ":party_no", skl_trade), # trade skill only affect sell price
	   # (store_sub, ":penality", 50, ":trade_skill"),
	   # (val_mul, ":old_item_price", 20),
	   # (val_div, ":old_item_price", ":penality"),
	   # (val_add, ":money_carried", ":old_item_price"),
	   
	   # (troop_add_item, ":troop_no", ":best_weapon"),
	   
	   # (this_or_next|le, ":weapon_to_remove", 0),
	   # (troop_remove_item, ":troop_no", ":weapon_to_remove"),
	   
	   # (troop_set_slot, ":troop_no", slot_troop_gold_carried, ":money_carried"),
...
#Recruiter kit end
 ...
 [
  ...
  # Constable training
   ...
   (store_skill_level, ":trainer_level", skl_trainer_replacement, "trp_player"),
	  ...
	  (try_begin),
	    ...
	    (try_begin),
		  ...
		  (else_try),
			...
			(troop_get_upgrade_troop, ":upgrade_troop_0", ":troop_id" , 0),
			(try_begin),
				(neg|troop_is_guarantee_ranged, ":upgrade_troop_0"),
				(store_random_in_range, ":upgrade", 0, 2),
				(troop_get_upgrade_troop, ":upgrade_troop", ":troop_id" , ":upgrade"),
			(try_end),
		  (try_end),
	    (else_try),
  ...
  # Patrol ai
   ...
   [
    ...
    (try_for_parties, ":party_no"),
      (party_slot_eq,":party_no", slot_party_type, spt_patrol),
	  (party_slot_ge, ":party_no", dplmc_slot_party_mission_diplomacy, 1),
